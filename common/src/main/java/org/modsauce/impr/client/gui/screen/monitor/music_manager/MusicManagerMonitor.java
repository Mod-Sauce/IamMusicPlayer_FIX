  ...
  extends Monitor<MusicManagerBlockEntity>
{
  ...
  public void init(int leftPos, int topPos) {
    ...
    // Image widgets are temporarily disabled for Minecraft 1.21+.
    // The previous ImageButton constructors that accepted explicit texture coordinates
    // (xTexStart, yTexStart, textureWidth, textureHeight) were replaced by constructors
    // that take a WidgetSprites instance. Re-enabling these buttons requires creating
    // WidgetSprites from `WIDGETS_TEXTURE` and using the appropriate ImageButton overload.
    //
    // Example (conceptual — adapt to the actual API available in your environment):
    // WidgetSprites sprites = new WidgetSprites(WIDGETS_TEXTURE, WIDGETS_TEXTURE);
    // addRenderWidget(new ImageButton(getStartX() + 356, getStartY(), 14, 10, sprites, button -> {
    //     insMonitor(MusicManagerBlockEntity.MonitorType.PLAY_LIST);
    // }, Component.translatable("imp.button.close")));
    //
    // Important: The old code relied on explicit source-rectangle coordinates in the texture.
    // With WidgetSprites you must handle sprite regions or perform custom rendering if you
    // need the same fine-grained control over texture coordinates. Keeping these widgets
    // commented out avoids incorrect rendering until the new sprite-based setup is applied.
  }
  ...
  public static MusicManagerMonitor createdMusicMonitor(
    MusicManagerBlockEntity.MonitorType type,
    MusicManagerScreen screen
  ) {
