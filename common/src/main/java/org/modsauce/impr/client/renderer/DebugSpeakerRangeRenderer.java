package org.modsauce.impr.client.renderer;
...
import org.modsauce.impr.api.client.MusicPlayerAccess;
import org.modsauce.impr.api.client.MusicSpeakerAccess;
...
public class DebugSpeakerRangeRenderer {
  ...
  private static void renderSpeaker(
    PoseStack poseStack,
    MultiBufferSource multiBufferSource,
    double camX,
    double camY,
    double camZ,
    MusicPlayerAccess playerAccess,
    MusicSpeakerAccess speaker
  ) {
    ...
    renderLineSphere(
      ...
      z,
      range,
      Mth.clamp((int) range, 3, 50),
      !playerAccess.isLoaded()
        ? 0xFFFFFF00
        : (inRange ? 0xFF00FF00 : 0xFFFF0000)
    );
  }

  private static void renderLineSphere(
    PoseStack poseStack,
  ...
  private static void renderLineQuarterSphereX(
    PoseStack poseStack,
    VertexConsumer vertexConsumer,
    float x,
    float y,
  ...
  private static void renderLineQuarterSphereY(
    PoseStack poseStack,
    VertexConsumer vertexConsumer,
    float x,
    float y,
  ...
  private static void renderLineQuarterSphereZ(
    PoseStack poseStack,
    VertexConsumer vertexConsumer,
    float x,
    float y,
  ...
  private static void renderLine(
    ...
    // NOTE: The VertexConsumer API changed around Minecraft 1.21. The previous
    // chainable pattern (e.g. addVertex(...).setColor(...).setNormal(...)) may
    // no longer match the upstream signatures. When porting to 1.21+, update
    // these calls to the new VertexConsumer method sequence (for example
    // separate `vertex(...)`, `color(...)`, `normal(...)`, `endVertex()` calls
    // or the appropriate replacements used by the current mappings).
    // Keep the current calls as-is for older mappings until the API is adapted.
    vertexConsumer
      .addVertex(pose.pose(), stX + x, stY + y, stZ + z)
      .setColor(r, g, b, a)
      .setNormal(pose, nx, ny, nz);
    vertexConsumer
      .addVertex(pose.pose(), enX + x, enY + y, enZ + z)
      .setColor(r, g, b, a)
      .setNormal(pose, nx, ny, nz);
  }
