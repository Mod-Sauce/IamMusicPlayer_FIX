...
public class IamMusicPlayerAPIImpl implements IamMusicPlayerAPI {
  ...
  public static final IamMusicPlayerAPI INSTANCE =
    new IamMusicPlayerAPIImpl();

  @Unmodifiable
  @Override
  public Collection<MusicRingerAccess> getRingers() {
    return MusicRingManager.getInstance()
      .getMusicRingers()
      .values()
      .stream()
      .flatMap(n -> n.getRingers().values().stream())
      .map(n -> (MusicRingerAccess) n)
      .toList();
  }
