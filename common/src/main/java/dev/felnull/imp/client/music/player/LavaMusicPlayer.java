...
public class LavaMusicPlayer extends BaseMusicPlayer {
  ...
  public LavaMusicPlayer(
    ...
    super(
      musicPlayerId,
      new AudioInfo(
        LavaPlayerManager.getInstance().getChannel(),
        LavaPlayerManager.getInstance().getSampleRate(),
        LavaPlayerManager.getInstance().getBit()
      ),
      musicSource,
      audioTrack.getInfo().isStream ? 5 : 16
    );
    this.audioTrack = audioTrack;
  }

  @Override
  protected AudioInputStream openAudioStream(long position)
    throws Exception {
      ...
      new AudioEventAdapter() {
        ...
        public void onTrackException(
          ...
          MusicEngine.getInstance()
            .getLogger()
            .error(
              "Audio track error: " + track.getInfo().identifier,
              exception
            );
          stopReadStream();
        }
    ...
    var stream = AudioPlayerInputStream.createStream(
      audioPlayer,
      lm.getAudioDataFormat(),
      1000 * 3,
      true
    );
