...
public class BoomboxButton extends IMPButton {
  ...
  public BoomboxButton(
    int x,
    int y,
    BoomboxData.ButtonType type,
    OnPress onPress,
    Supplier<BoomboxData.Buttons> buttons
  ) {
  ...
  public void renderWidget(
    ...
    if (type.getState(buttons.get())) r += 2;
    OERenderUtils.drawTexture(
      BoomboxScreen.BG_TEXTURE,
      guiGraphics.pose(),
      getX(),
      getY(),
      r * 19,
    ...
    OERenderUtils.drawTexture(
      ...
      getY() + zy,
      (type.ordinal() - 1) * 9,
      188 + (downShift.getAsBoolean() ? 9 : 0),
      9,
      9
    );
  }
