...
public class MusicUtils {
  ...
  public static void checkALError() {
    ...
    switch (e) {
      ...
      case AL10.AL_INVALID_VALUE -> error =
        "Invalid enum parameter value";
      case AL10.AL_INVALID_OPERATION -> error = "Illegal call";
      case AL10.AL_OUT_OF_MEMORY -> error =
        "Unable to allocate memory";
    }
    if (e != AL10.AL_NO_ERROR) error = "Unknown error";

    if (error != null) MusicEngine.getInstance()
      .getLogger()
      .error("OpenAL Error: {}", error);
  }
  ...
  public static void assertOnMusicTick() {
    if (!isOnMusicTick()) throw new RuntimeException(
  ...
  public static void runOnMusicTick(Runnable runnable) {
    if (isOnMusicTick()) {
      ...
      CompletableFuture.runAsync(
        runnable,
        MusicEngine.getInstance().getMusicTickExecutor()
      ).join();
    }
  }

  public static void runInvokeTasks(
    InvokeExecutor executor,
    String name
  ) {
    executor.runTasks(100);
    if (executor.getTaskCount() != 0) MusicEngine.getInstance()
      .getLogger()
      .warn(
        "A lot of music processing tasks are running,Execute separately to reduce the load - Remaining " +
          name +
          " Tasks:" +
  ...
  public static boolean isSpatial(SpatialType spatialType) {
    if (
      spatialType == SpatialType.ENTRUST
