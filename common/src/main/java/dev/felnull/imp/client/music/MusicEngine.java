...
public class MusicEngine implements MusicEngineAccess {
  ...
  private final InvokeExecutor musicTickExecutor =
    new InvokeExecutor();
  private ExecutorService musicLoaderExecutor =
    createMusicLoadExecutor();
  private MusicEngineDestroyRunner musicDestroyRunner =
    new MusicEngineDestroyRunner();
  ...
  public String getDebugString() {
    return String.format(
      "[%s] SPK: %d/%d, LOAD: %d/%d, WKR: %d, TASK %d, TICK: %d ms",
      IamMusicPlayer.getModName(),
      getCurrentMusicSpeaker(),
      getMaxMusicSpeaker(),
      getCurrentMusicLoad(),
      getMaxMusicLoad(),
      getCurrentMusicLoader(),
      getCurrentMusicTaskCount(),
      lastProsesTime
    );
  }
  ...
  public int getCurrentMusicLoader() {
    if (
      musicLoaderExecutor instanceof
  ...
  private ExecutorService createMusicLoadExecutor() {
    return Executors.newCachedThreadPool(
      new BasicThreadFactory.Builder()
        .namingPattern("IMP-Music-Loader-%d")
        .daemon(true)
        .build()
    );
  }
   ...
   */
  public boolean load(
    @NotNull UUID musicPlayerId,
    @NotNull MusicSource source,
    long position,
    @NotNull LoadCompleteListener listener
  ) {
    if (getCurrentMusicLoad() >= getMaxMusicLoad()) return false;

    if (isExist(musicPlayerId)) return false;
  ...
  public boolean play(@NotNull UUID musicPlayerId, long delay) {
    ...
    if (mpe.getMusicPlayer() != null) {
      for (MusicSpeaker musicSpeaker : mpe
        .getMusicPlayer()
        .getSpeakerList()) {
        notifyRangeEntities(
          musicPlayerId,
          musicSpeaker.getTracker().getSpeakerInfo()
        );
      }
    }
  ...
  public boolean addSpeaker(
    ...
    if (
      getCurrentMusicSpeaker() >= getMaxMusicSpeaker()
    ) return false;
  ...
  public Map<UUID, MusicPlayerAccess> getMusicPlayers() {
    ImmutableMap.Builder<
      UUID,
      MusicPlayerAccess
    > musicPlayersBuilder = ImmutableMap.builder();
    for (Map.Entry<
      UUID,
      MusicEntry
    > entry : musicEntries.entrySet()) {
      var player = entry.getValue().getMusicPlayer();
      if (player != null) musicPlayersBuilder.put(
        entry.getKey(),
        player
      );
    }
  ...
  public boolean loadAndPlaySimple(
    @NotNull UUID musicPlayerId,
    @NotNull MusicTracker musicTracker,
    @NotNull MusicSource source,
    long position,
    boolean delayed
  ) {
    ...
    if (ret) addSpeaker(musicPlayerId, musicPlayerId, musicTracker);
    return ret;
  }
   ...
   * @return 更新できたかどうか
   */
  public boolean updateMusicTracker(
    @NotNull UUID musicPlayerId,
    @NotNull UUID speakerId,
    MusicTracker musicTracker
  ) {
    if (musicTracker == null) return false;
  ...
  private void notifyRangeEntities(
    UUID uuid,
    MusicSpeakerInfo speakerInfo
  ) {
    if (mc.level == null) return;

    var v3 = speakerInfo.position();
    var aabb = new AABB(BlockPos.containing(v3)).inflate(
      speakerInfo.getRange()
    );
    for (LivingEntity livingentity : mc.level.getEntitiesOfClass(
      LivingEntity.class,
      aabb
    )) {
      if (
