...
public class LavaPlayerManager {
  ...
  private static final LavaPlayerManager INSTANCE =
    new LavaPlayerManager();
  private final Map<String, LavaPlayerBaseMusicMedia> medias =
  ...
  public void reload() {
    ...
    if (audioPlayerManager != null) audioPlayerManager.shutdown();

    audioDataFormat = new Pcm16AudioDataFormat(
      2,
      IamMusicPlayer.getConfig().sampleRate,
      960,
      false
    );
  ...
  private AudioPlayerManager createAudioPlayerManager() {
    ...
    audioPlayerManager
      .getConfiguration()
      .setOutputFormat(getAudioDataFormat());
  ...
  private void registerSourceManager(
    AudioPlayerManager audioPlayerManager
  ) {
    Comparator<Map.Entry<String, MusicMedia>> sc =
      Comparator.comparingInt(value -> {
        if (
          value.getValue() instanceof
            LavaPlayerBaseMusicMedia lavaPlayerBaseMusicMedia
        ) return lavaPlayerBaseMusicMedia.priority();
        return 0;
      });

    IMPMusicMedias.getAllMedia()
      .entrySet()
      .stream()
      .sorted(sc.reversed())
      .forEach(v -> {
        ...
        if (
          ...
          lavaPlayerBaseMusicMedia.registerSourceManager(
            audioPlayerManager
          );
  ...
  public Optional<AudioTrack> loadTrack(String identifier)
  ...
  @Nullable
  public Pair<MusicMedia, MusicMediaResult> autoLoad(
    String sourceName
  )
    throws ExecutionException, InterruptedException, TimeoutException {
    ...
    if (
      track.isEmpty() || track.get().getInfo().isStream
    ) return null;

    for (LavaPlayerBaseMusicMedia value : medias.values()) {
      if (value.match(track.get())) return Pair.of(
        value,
        value.createResult(track.get())
      );
    }
    return null;
  }

  public List<AudioTrack> searchYoutube(String name)
    throws ExecutionException, InterruptedException {
    return loadTracks("ytsearch:" + name).getRight();
  }

  public Pair<AudioPlaylist, List<AudioTrack>> loadTracks(String name)
    throws ExecutionException, InterruptedException {
