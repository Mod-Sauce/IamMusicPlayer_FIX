...
public class AuthorityMMMonitor extends MusicManagerMonitor {
  ...
  private static final ResourceLocation AUTHORITY_TEXTURE =
    new ResourceLocation(
      IamMusicPlayer.MODID,
      "textures/gui/container/music_manager/monitor/authority.png"
    );
  private static final Component BACK_TEXT = Component.translatable(
    "gui.back"
  );
  private static final Component EXPULSION_BUTTON_TEXT =
    Component.translatable("imp.button.expulsion").withStyle(
      ChatFormatting.DARK_RED
    );
  private static final Component CANT_CHANGE_AUTHORITY =
    Component.translatable("imp.text.cantChangeAuthority");
  private static final Component READONLY_RDO_TEXT =
    Component.translatable("imp.radioButton.readonly");
  private static final Component MEMBER_RDO_TEXT =
    Component.translatable("imp.radioButton.member");
  private static final Component BAN_RDO_TEXT =
    Component.translatable("imp.radioButton.ban");
  private static final Component ADMIN_RDO_TEXT =
    Component.translatable("imp.radioButton.admin");
  ...
  public void init(int leftPos, int topPos) {
    ...
    this.authorityPlayersFixedButtonsList = addRenderWidget(
      new AuthorityPlayersFixedListWidget(
        getStartX() + 6,
        getStartY() + 23,
        175,
        135,
        Component.translatable("imp.fixedList.authorityPlayers"),
        9,
        members,
        (widget, item) -> setSelectedPlayer(item),
        this.authorityPlayersFixedButtonsList,
        n -> n.equals(getSelectedPlayer())
      )
    );

    Supplier<Set<RadioButton>> rdos = () ->
      ImmutableSet.of(
        this.readOnlyRadio,
        this.memberRadio,
        this.adminRadio,
    ...
    this.readOnlyRadio = this.addRenderWidget(
      new SmartRadioButton(
        getStartX() + 188,
        getStartY() + 47,
        READONLY_RDO_TEXT,
        n ->
          sendChangeAuthorityPacket(
            getSelectedPlayer(),
            AuthorityInfo.AuthorityType.READ_ONLY
          ),
        rdos
      )
    );
    this.readOnlyRadio.setSelected(
      getAuthorityType(getSelectedPlayer()) ==
        AuthorityInfo.AuthorityType.READ_ONLY
    );

    this.memberRadio = this.addRenderWidget(
      new SmartRadioButton(
        getStartX() + 188,
        getStartY() + 72,
        MEMBER_RDO_TEXT,
        n ->
          sendChangeAuthorityPacket(
            getSelectedPlayer(),
            AuthorityInfo.AuthorityType.MEMBER
          ),
        rdos
      )
    );
    this.memberRadio.setSelected(
      getAuthorityType(getSelectedPlayer()) ==
        AuthorityInfo.AuthorityType.MEMBER
    );

    this.banRadio = this.addRenderWidget(
      new SmartRadioButton(
        getStartX() + 188,
        getStartY() + 97,
        BAN_RDO_TEXT,
        n ->
          sendChangeAuthorityPacket(
            getSelectedPlayer(),
            AuthorityInfo.AuthorityType.BAN
          ),
        rdos
      )
    );
    this.banRadio.setSelected(
      getAuthorityType(getSelectedPlayer()) ==
        AuthorityInfo.AuthorityType.BAN
    );

    this.adminRadio = this.addRenderWidget(
      new SmartRadioButton(
        getStartX() + 188,
        getStartY() + 122,
        ADMIN_RDO_TEXT,
        n ->
          sendChangeAuthorityPacket(
            getSelectedPlayer(),
            AuthorityInfo.AuthorityType.ADMIN
          ),
        rdos
      )
    );
    this.adminRadio.setSelected(
      getAuthorityType(getSelectedPlayer()) ==
        AuthorityInfo.AuthorityType.READ_ONLY
    );
    ...
    this.adminRadio.visible =
      canEdit(getSelectedPlayer()) && canSetAdmin();

    this.expulsionButton = addRenderWidget(
      new SmartButton(
        getStartX() + 188,
        getStartY() + 147,
        87,
        15,
        EXPULSION_BUTTON_TEXT,
        n -> {
          sendChangeAuthorityPacket(
            getSelectedPlayer(),
            AuthorityInfo.AuthorityType.NONE
          );
  ...
  public void render(
    ...
    OERenderUtils.drawTexture(
      ...
      0f,
      0f,
      width / (getSelectedPlayer() != null ? 1f : 2f),
      height,
      width,
      height
    );
    ...
    if (sp != null) {
      ...
      var str = OEClientUtils.getPlayerNameByUUID(sp).orElseGet(
        sp::toString
      );
      drawSmartFixedWidthText(
        guiGraphics,
      ...
      drawSmartFixedWidthText(
        guiGraphics,
        getAuthorityType(sp).getText(),
        getStartX() + 212,
        getStartY() + 35,
        150
      );
      if (!canEdit(sp)) drawSmartText(
        guiGraphics,
        CANT_CHANGE_AUTHORITY,
        getStartX() + 188,
        getStartY() + 47
      );
    }
  ...
  public void renderAppearance(
    ...
    OERenderUtils.renderTextureSprite(
      ...
      0,
      monitorWidth /
        (getSelectedPlayer(blockEntity) != null ? 1f : 2f),
      monitorHeight,
      0,
      0,
      width / (getSelectedPlayer(blockEntity) != null ? 1f : 2f),
      height,
      width,
      height,
      i,
    ...
    if (sp != null) {
      ...
      renderSmartTextSprite(
        ...
        multiBufferSource,
        Component.literal(
          OEClientUtils.getWidthOmitText(str, 130, "...")
        ),
        215,
        26,
        OERenderUtils.MIN_BREADTH * 4,
      ...
      renderSmartTextSprite(
        poseStack,
        multiBufferSource,
        getAuthorityType(blockEntity, sp).getText(),
        215,
        36,
        OERenderUtils.MIN_BREADTH * 4,
        onPxW,
    ...
    if (pl != null) pls.addAll(
      pl
        ...
        .filter(
          n ->
            n.getValue() != AuthorityInfo.AuthorityType.NONE &&
            n.getValue() != AuthorityInfo.AuthorityType.INVITATION
        )
        .map(Map.Entry::getKey)
        .toList()
    );

    renderFixedListSprite(
      poseStack,
      multiBufferSource,
      6,
      23,
      ...
      (
        ...
        renderSmartButtonBoxSprite(
          ...
          onPxH,
          monitorHeight,
          entry.equals(getSelectedPlayer(blockEntity))
        );
        renderPlayerFaceSprite(
          poseStack1,
          multiBufferSource1,
        ...
        renderSmartTextSprite(
          ...
          multiBufferSource1,
          Component.literal(
            OEClientUtils.getWidthOmitText(
              OEClientUtils.getPlayerNameByUUID(entry).orElseGet(
                entry::toString
              ),
              w - (h + 7),
              "..."
  ...
  private void sendChangeAuthorityPacket(
    UUID playerID,
    AuthorityInfo.AuthorityType type
  ) {
    if (playerID == null) return;
    var ps = getSelectedMusicPlayList();
    if (ps != null) NetworkManager.sendToServer(
      IMPPackets.MUSIC_PLAYLIST_CHANGE_AUTHORITY,
      new IMPPackets.MusicPlayListChangeAuthorityMessage(
        ps.getUuid(),
        playerID,
        type,
        BlockEntityExistence.getByBlockEntity(
          getScreen().getBlockEntity()
        )
      ).toFBB()
    );
  }

  protected AuthorityInfo.AuthorityType getAuthorityType(
    UUID playerId
  ) {
    if (
  ...
  private boolean canSetAdmin(
    ...
    if (pl != null) {
      return pl
        .getAuthority()
        .getAuthorityType(mc.player.getGameProfile().getId())
        .isMoreOwner();
    }
    return false;
  }
  ...
  protected MusicPlayList getSelectedMusicPlayList(
    ...
    return pls
      ...
      .filter(n ->
        n
          .getUuid()
          .equals(getSelectedPlayList(musicManagerBlockEntity))
      )
      .findFirst()
      .orElse(null);
  }
