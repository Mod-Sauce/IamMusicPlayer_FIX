...
public class SearchMusicMMMonitor extends MusicManagerMonitor {
  ...
  private static final ResourceLocation SEARCH_MUSIC_TEXTURE =
    new ResourceLocation(
      IamMusicPlayer.MODID,
      "textures/gui/container/music_manager/monitor/search_music.png"
    );
  private static final Component SEARCHING_TEXT =
    Component.translatable("imp.text.searching");
  ...
  public void init(int leftPos, int topPos) {
    ...
    this.searchMusicsFixedButtonsList = this.addRenderWidget(
      new SearchMusicsFixedListWidget(
        getStartX() + 2,
        getStartY() + 25,
        366,
        172,
        Component.translatable("imp.fixedList.searchMusic"),
        4,
        searchMusics,
        (widget, item) -> {
    ...
    this.searchNameEditBox = new EditBox(
      IIMPSmartRender.mc.font,
      getStartX() + 2,
      getStartY() + 11,
      367,
      12,
      Component.translatable("imp.editBox.musicSearchName")
    );
  ...
  public void render(
    ...
    if (
      searchMusics.isEmpty() &&
      !getMusicSearchName().isEmpty() &&
      searchThread != null &&
  ...
  public void renderAppearance(
    ...
    renderSmartEditBoxSprite(
      ...
      onPxH,
      monitorHeight,
      getMusicSearchName(blockEntity)
    );
  ...
  private void setMusicSearchName(String name) {
    if (!getMusicSearchName().equals(name)) startMusicSearch(name);
  ...
  private class SearchThread extends FlagThread {
    ...
    public void run() {
      ...
      if (lt == null || !lt.isSearchable()) return;
    ...
    private synchronized void setSearchMusics(
      List<MusicMediaResult> musics
    ) {
      if (musics != null) {
        searchMusics.clear();
