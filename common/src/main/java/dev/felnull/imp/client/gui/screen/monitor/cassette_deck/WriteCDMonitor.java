...
public class WriteCDMonitor extends CassetteDeckMonitor {
  ...
  protected static final ResourceLocation WRITE_TEXTURE =
    new ResourceLocation(
      IamMusicPlayer.MODID,
      "textures/gui/container/cassette_deck/monitor/write.png"
    );
  ...
  public void init(int leftPos, int topPos) {
    ...
    this.writeButton = this.addRenderWidget(
      new SmartButton(
        getStartX() + 164,
        getStartY() + 44,
        35,
        11,
        Component.translatable("imp.button.writeStart"),
        n ->
          insMonitor(
            CassetteDeckBlockEntity.MonitorType.WRITE_EXECUTION
          )
      )
    );
    this.writeButton.setHideText(true);
    this.writeButton.setIcon(
      MusicManagerMonitor.WIDGETS_TEXTURE,
      11,
      131,
      20,
    ...
    this.writePlayListFixedButtonsList = this.addRenderWidget(
      new WritePlayListFixedListWidget(
        getStartX() + 1,
        getStartY() + 1,
        musicPlayLists,
        (widget, item) -> setSelectPlaylist(item.getUuid()),
        this.writePlayListFixedButtonsList,
        n -> n.getUuid().equals(getSelectPlaylist())
      )
    );

    this.writeMusicsFixedButtonsList = this.addRenderWidget(
      new WriteMusicsFixedListWidget(
        getStartX() + 70,
        getStartY() + 1,
        musics,
        (widget, item) -> setMusic(item.getUuid()),
        n -> n.equals(getMusic()),
        this.writeMusicsFixedButtonsList
      )
    );
  }
  ...
  public void renderAppearance(
    ...
    renderFixedListSprite(
      ...
      (
        ...
        renderSmartButtonBoxSprite(
          ...
          onPxH,
          monitorHeight,
          entry.getUuid().equals(getSelectPlaylist(blockEntity))
        );
        float sx = 1;
    ...
    renderFixedListSprite(
      ...
      onPxH,
      monitorHeight,
      getSyncManager().getMusics(getSelectPlaylist(blockEntity)),
      6,
      (
        poseStack12,
        multiBufferSource12,
        ...
        renderSmartButtonBoxSprite(
          ...
          onPxH,
          monitorHeight,
          getMusic(blockEntity) != null &&
            entry.getUuid().equals(getMusic(blockEntity).getUuid())
        );
        float sx = 1;
        var img = entry.getImage();
        if (!img.isEmpty()) {
  ...
  private MusicPlayList getRawSelectPlaylist() {
    if (
      getBlockEntity() instanceof
        CassetteDeckBlockEntity cassetteDeckBlockEntity
  ...
  private MusicPlayList getRawSelectPlaylist(
    ...
    if (pls != null) return getSyncManager()
      .getMyPlayList()
      .stream()
      .filter(n -> n.getUuid().equals(pls))
      .findFirst()
      .orElse(null);
    return null;
  }
  ...
  private boolean canWriteStart(
    ...
    return (
      !tape.isEmpty() &&
      IMPItemUtil.isCassetteTape(tape) &&
      getMusic(cassetteDeckBlockEntity) != null
    );
  }
