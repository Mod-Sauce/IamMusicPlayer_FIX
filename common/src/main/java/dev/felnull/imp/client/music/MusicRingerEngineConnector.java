...
public class MusicRingerEngineConnector {
  ...
  public static IMPPackets.MusicRingResponseStateType update(
    UUID musicPlayerId,
    MusicTracker musicTracker
  ) {
    if (getEngine().isPlaying(musicPlayerId)) {
      getEngine().updateMusicTracker(
        musicPlayerId,
        musicPlayerId,
        musicTracker
      );
    ...
    if (
      getEngine().isLoading(musicPlayerId) ||
      getEngine().isExist(musicPlayerId)
