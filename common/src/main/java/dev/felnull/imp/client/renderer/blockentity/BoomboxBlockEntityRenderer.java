  ...
  extends AbstractBlockEntityRenderer<BoomboxBlockEntity>
{
  ...
  protected BoomboxBlockEntityRenderer(
  ...
  public void render(
    ...
    renderBoombox(
      poseStack,
      multiBufferSource,
      state.getValue(MusicManagerBlock.FACING),
      i,
      j,
      f,
      data,
      data.getHandleRaisedProgress(f) /
        (float) data.getHandleRaisedMax(),
      multiBufferSource.getBuffer(Sheets.cutoutBlockSheet())
    );
  }

  public static void renderBoombox(
    PoseStack poseStack,
    MultiBufferSource multiBufferSource,
    Direction direction,
    int i,
    ...
    float lidOpen =
      data.getLidOpenProgress(f) /
      (float) data.getLidOpenProgressMax();
    ...
    if (lidOpen != 0) {
      ...
      mc
        .getItemRenderer()
        .renderStatic(
          changeCassetteTape ? oldCassetteTape : cassetteTape,
    ...
    poseStack.popPose();
    poseStack.pushPose();
    OERenderUtils.poseTrans16(
      poseStack,
      11.25,
      9 - (buttons.radio() ? 0.5 : 0),
      5.75
    );
    OERenderUtils.renderModel(
      poseStack,
  ...
  private static void renderAntenna(
    PoseStack poseStack,
    MultiBufferSource multiBufferSource,
    int i,
    int j,
    ...
    if (antenna.is(IMPItems.RADIO_ANTENNA.get())) {
      ...
      AntennaItemRenderer.renderAntenna(
        antenna,
        poseStack,
        multiBufferSource,
        i,
        j,
        (-0.5f + Math.max(antennaPar, 0.5f)) * 2f,
        -90 + 30 * Math.min(antennaPar, 0.5f) * 2f
      );
      poseStack.popPose();
    } else {
      poseStack.pushPose();
      ...
      OERenderUtils.poseRotateX(
        poseStack,
        -30 + Math.abs(-0.5f + ((roted % 120) / 120f)) * 2f * 60
      );
      OERenderUtils.poseTrans16(poseStack, 0, -1.3, 0);
      mc
        .getItemRenderer()
  ...
  private static BoomboxMonitor getMonitor(
    BoomboxData.MonitorType type
  ) {
