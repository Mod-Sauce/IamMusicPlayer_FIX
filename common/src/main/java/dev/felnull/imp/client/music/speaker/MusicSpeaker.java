...
public class MusicSpeaker implements MusicSpeakerAccess {
  ...
  public boolean canPlay() {
    return (
      !isDead() &&
      !buffers.isEmpty() &&
      getLiveTime() >= scheduledStartTime
    );
  }

  public boolean isPlaying() {
    return (
      getPlayState() == AL_PLAYING || getPlayState() == AL_PAUSED
    );
  }
  ...
  public void tick() {
    ...
    if (
      isPlaying() &&
      SoundPhysicsRemasteredIntegration.INSTANCE.isEnable()
    ) SoundPhysicsRemasteredIntegration.INSTANCE.onSound(
      musicPlayerId,
      speakerId,
      source,
      MusicUtils.isSpatial(fixedInfo.spatialType())
        ? tracker.getSpeakerInfo().getPosition()
        : null
    );
   ...
   */
  public void resume() {
    if (this.pauseTime >= 0) this.totalPauseTime +=
  ...
  public void destroy() throws Exception {
    ...
    if (
      SoundPhysicsRemasteredIntegration.INSTANCE.isEnable()
    ) SoundPhysicsRemasteredIntegration.INSTANCE.onDestroy(
      musicPlayerId,
      speakerId
    );
  ...
  public void stop() {
    ...
    if (getPlayState() != AL_STOPPED) stopTime =
  ...
  public void setTracker(MusicTracker tracker) {
    this.tracker = tracker;
    if (
      !this.fixedInfo.equals(
        this.tracker.getSpeakerInfo().fixedInfo()
      )
    ) {
      stop();
      return;
  ...
  private void alSourceUpdate() {
    ...
    if (sp) {
      alSource3f(
        source,
        AL_POSITION,
        (float) spi.position().x(),
        (float) spi.position().y(),
        (float) spi.position().z()
      );
      alSourcef(
        source,
        AL_GAIN,
        (float) MusicMath.calculateVolume(spi.volume())
      );
      ...
      alSourcef(
        source,
        AL_GAIN,
        (float) MusicMath.calculateVolume(
          MusicMath.calculatePseudoAttenuation(
            spi.position(),
            spi.range(),
            spi.volume()
          )
