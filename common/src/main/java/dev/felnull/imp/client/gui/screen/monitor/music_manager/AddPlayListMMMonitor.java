...
public class AddPlayListMMMonitor extends MusicManagerMonitor {
  ...
  private static final ResourceLocation ADD_PLAY_LIST_TEXTURE =
    new ResourceLocation(
      IamMusicPlayer.MODID,
      "textures/gui/container/music_manager/monitor/add_play_list.png"
    );
  private static final Component CREATE_PLAYLIST_TEXT =
    Component.translatable("imp.button.createPlaylist");
  private static final Component ONLINE_PLAYLIST_TEXT =
    Component.translatable("imp.button.addOnlinePlaylist");
  private final List<MusicPlayList> musicPlayLists =
    new ArrayList<>();
  private final Component PUBLIC_TEXT = Component.translatable(
    "imp.text.public"
  );
  ...
  public void init(int leftPos, int topPos) {
    ...
    this.joinPlayListFixedButtonsList = addRenderWidget(
      new JoinPlayListFixedListWidget(
        getStartX() + 1,
        getStartY() + 20,
        musicPlayLists,
        (widget, item) -> {
    ...
    this.createPlayListButton = addRenderWidget(
      new SmartButton(
        getStartX() + 1,
        getStartY() + 189,
        90,
    ...
    this.addOnlinePlayListButton = addRenderWidget(
      new SmartButton(
        getStartX() + 91,
        getStartY() + 189,
        122,
        9,
        ONLINE_PLAYLIST_TEXT,
        n ->
          insMonitor(
    ...
    this.addOnlinePlayListButton.setIcon(
      WIDGETS_TEXTURE,
      83,
      14,
      5,
      ...
      for (int k = 0; k < Math.min(6, pls.size()); k++) {
        ...
        if (!playList.getImage().isEmpty()) {
          ...
          PlayImageRenderer.getInstance().renderSprite(
            ...
            multiBufferSource,
            3 * onPxW,
            monitorHeight - (20 + (k * 28) + 2 + 26) * onPxH,
        ...
        renderSmartTextSprite(
          poseStack,
          multiBufferSource,
          Component.literal(playList.getName()),
          sx + 3,
          20 + (k * 28) + 5,
          OERenderUtils.MIN_BREADTH * 4,
          onPxW,
        ...
        poseStack.translate(
          (sx + 101) * onPxW,
          monitorHeight - (20 + (k * 28) + 2 + 9) * onPxH,
          OERenderUtils.MIN_BREADTH * 4
        );
        OERenderUtils.renderPlayerFaceSprite(
          poseStack,
          multiBufferSource,
          playList.getAuthority().getOwnerName(),
          onPxH * 9,
          i,
          j
        );
        ...
        renderSmartTextSprite(
          poseStack,
          multiBufferSource,
          Component.literal(playList.getAuthority().getOwnerName()),
          sx + 114,
          20 + (k * 28) + 5,
          OERenderUtils.MIN_BREADTH * 4,
          onPxW,
        ...
        renderSmartTextSprite(
          ...
          multiBufferSource,
          Component.translatable(
            "imp.text.playerCount",
            playList.getPlayerCount()
          ),
          sx + 156,
          20 + (k * 28) + 18,
          OERenderUtils.MIN_BREADTH * 4,
        ...
        if (
          ...
          renderSmartTextSpriteColorSprite(
            poseStack,
            multiBufferSource,
            Component.translatable("imp.text.invitation"),
            sx + 208,
            20 + (k * 28) + 5,
            OERenderUtils.MIN_BREADTH * 4,
            onPxW,
    ...
    if (pls.getCanJoinPlayListInfo() != lastPlayListInfo) {
      ...
      if (lastPlayListInfo != null) {
        INFO_TEXT = Component.translatable(
          "imp.text.addPlaylistInfo",
          lastPlayListInfo.playListCount(),
          lastPlayListInfo.playerCount(),
