...
public abstract class BaseMusicPlayer
  implements
    MusicPlayer<BaseMusicPlayer.LoadInput, BaseMusicPlayer.LoadResult>
{
  ...
  public List<MusicSpeaker> getSpeakerList() {
    ImmutableList.Builder<MusicSpeaker> speakerBuilder =
  ...
  public void play(long delay) {
    ...
    if (pause) this.pauseTime = System.currentTimeMillis();
      ...
      () -> {
        try {
          ...
          MusicEngine.getInstance()
            .getLogger()
            .error("Failed to close music stream", e);
        }
      },
  ...
  @Override
  public void pause() {
    if (playing) this.pauseTime = System.currentTimeMillis();
  ...
  public void tick() {
    ...
    speakers.forEach((id, speaker) -> {
      if (speaker.isDead()) {
        try {
          ...
          MusicEngine.getInstance()
            .getLogger()
            .error("Failed to destroy speaker", e);
        }
        delSpeakers.add(id);
        return;
      }
    ...
    for (ReadEntry readEntry : readEntries) {
      ...
      for (Map.Entry<
        MusicSpeakerFixedInfo,
        MusicBuffer
      > entry : readEntry.buffers.entrySet()) {
    ...
    for (UUID delSpeaker : delSpeakers) {
      preSpeakers.put(
        ...
        new MusicSpeaker(
          musicPlayerId,
          delSpeaker,
          speakers.get(delSpeaker).getTracker()
        )
      );
  ...
  private void readTick() {
    ...
    if (musicSource.isLive()) {
      ...
      nc = (int) ((float) (musicSource.getDuration() -
            getPosition()) /
          1000f +
        0.5f);
    }
    ...
    if (
      ...
      !loadEnd &&
      !reading &&
      (lc <= aheadLoad || (nc - aheadLoad) <= aheadLoad)
    ) {
      ...
      cf.whenCompleteAsync(
        (ret, error) -> {
          ...
          MusicEngine.getInstance()
            .getLogger()
            .error("Failed to read audio data", error);
        },
        tickExecutor
      );
    }
  ...
  private void updatePreSpeaker(UUID uuid) {
    ...
    var r = readEntries
      .stream()
      .filter(readEntry -> readEntry.position() >= getPosition())
      .sorted((o1, o2) -> (int) (o1.position() - o2.position()))
      .toList();

    if (
      r
        .stream()
        .allMatch(n -> n.buffers.containsKey(sp.getFixedInfo()))
    ) {
      ...
      List<ReadEntry> noLoaded = r
        .stream()
        .filter(n -> !n.buffers.containsKey(sp.getFixedInfo()))
        .toList();
      for (ReadEntry readEntry : noLoaded) {
        ...
        cf.whenCompleteAsync(
          (ret, error) -> {
            ...
            MusicEngine.getInstance()
              .getLogger()
              .error("Failed to middle load audio data", error);
          },
          tickExecutor
        );
      }
  ...
  public boolean existSpeaker(UUID uuid) {
    if (speakers.containsKey(uuid)) return true;
  ...
  public LoadInput loadStart(long position) {
    ...
    return new LoadInput(
      position,
      ImmutableMap.copyOf(preSpeakers),
      aheadLoad,
      musicSource.isLive()
    );
  }

  protected abstract AudioInputStream openAudioStream(long position)
    throws Exception;
  ...
  public LoadResult loadAsync(LoadInput input) throws Exception {
    ...
    if (input.live()) Thread.sleep(1000 * 3);

    return new LoadResult(
      stream,
      readRet,
      input.speakers.keySet().stream().toList()
    );
  }
  ...
  public boolean waitDestroy() {
    return (
      finished ||
      (!musicSource.isLive() &&
        getPosition() >= musicSource.getDuration())
    );
  }
  ...
  public Map<UUID, MusicSpeakerAccess> getSpeakers() {
    ImmutableMap.Builder<UUID, MusicSpeakerAccess> speakersBuilder =
  ...
  private ReadInput readStart(int loadCount) {
    ...
    return new ReadInput(
      this.stream,
      this.totalReadTime + this.startPosition,
      loadCount,
      infos.stream().distinct().toList(),
      getAudioInfo()
    );
  }
  ...
  private boolean readStream(AudioInputStream stream, byte[] buffer)
    throws IOException {
    synchronized (readLock) {
      try {
  ...
  private ReadResult readAsync(ReadInput input) throws Exception {
    int ol =
      input.audioInfo().sampleRate() *
      input.audioInfo().channel() *
      (input.audioInfo().bit() / 8);
    ...
    for (int i = 0; i < input.aheadLoad(); i++) {
      ...
      if (
        !musicSource.isLive() &&
        (st + 1000) > musicSource.getDuration()
      ) {
      ...
      MusicInstantaneous mi = MusicInstantaneous.create(
        st,
        data,
        input.audioInfo().channel(),
        input.audioInfo.bit()
      ...
      Map<MusicSpeakerFixedInfo, BufferFactory> buffes =
        new HashMap<>();
      for (MusicSpeakerFixedInfo fixedInfo : input.fixedInfos()) {
        buffes.put(
          fixedInfo,
          BufferFactory.create(data, fixedInfo, input.audioInfo())
        );
      }
  ...
  private void insertBuffer(
    MusicSpeakerFixedInfo info,
    MusicBuffer buffer
  ) {
    for (MusicSpeaker value : speakers.values()) {
      if (
        !value.isDead() && value.getFixedInfo().equals(info)
  ...
  private MiddleLoadResult middleLoadAsync(MiddleLoadInput input) {
    ...
    return new MiddleLoadResult(
      input.readWaiter(),
      BufferFactory.create(
        data,
        input.readWaiter().fixedInfo(),
        input.audioInfo()
      )
    );
  }
  ...
  private void middleLoadApply(MiddleLoadResult result) {
    if (
      result.readWaiter.readEntry.position() < getPosition()
    ) return;

    result.readWaiter
      .readEntry()
      .buffers.put(
        result.readWaiter.fixedInfo,
        result.bufferFactory().create()
      );
  ...
  public List<MusicLoadChunk> getLoadChunks() {
    return readEntries
      .stream()
      .map(n -> new MusicLoadChunk(n.position(), 1000))
      .toList();
  }

  public static record LoadInput(
    long position,
    Map<UUID, MusicSpeaker> speakers,
    int aheadLoad,
    boolean live
  ) {}
  ...
  private static record ReadInput(
    AudioInputStream stream,
    long readStartPosition,
    int aheadLoad,
    List<MusicSpeakerFixedInfo> fixedInfos,
    AudioInfo audioInfo
  ) {}

  private static record ReadResult(
    List<ReadResultEntry> readEntries,
    boolean end
  ) {}

  private static record ReadResultEntry(
    long position,
    byte[] data,
    MusicInstantaneous instantaneous,
    ...
    private ReadEntry create() {
      ...
      buffers.forEach((fixedInfo, bufferFactory) ->
        rb.put(fixedInfo, bufferFactory.create())
      );
      return new ReadEntry(position, data, instantaneous, rb);
    }
  }

  private static record ReadEntry(
    long position,
    byte[] data,
    MusicInstantaneous instantaneous,
  ...
  private static record ReadWait(
    ReadEntry readEntry,
    MusicSpeakerFixedInfo fixedInfo
  ) {}

  private static record BufferFactory(
    ByteBuffer buffer,
    boolean relative,
    AudioInfo audioData
  ) {
    ...
    private static BufferFactory create(
      byte[] data,
      MusicSpeakerFixedInfo fixedInfo,
      AudioInfo audioInfo
    ) {
      if (
        MusicUtils.isSpatial(fixedInfo.spatialType()) &&
        fixedInfo.channel() <= -1 &&
        audioInfo.channel() >= 2
      ) {
        ...
        fixedInfo.channel() >= 0 && audioInfo.channel() >= 2
      ) {
    ...
    private static byte[] synthesisChannel(
      byte[] data,
      AudioInfo audioInfo
    ) {
      if (audioInfo.bit() != 16) throw new RuntimeException(
        "Unsupported bit"
      );
      ...
      for (
        int i = 0;
        i < data.length / 2 / audioInfo.channel();
        i++
      ) {
        float total = 0;
        for (int j = 0; j < audioInfo.channel(); j++) {
          ...
          short r = ByteBuffer.wrap(d)
            .order(ByteOrder.LITTLE_ENDIAN)
            .getShort();
          float val = (float) r / (float) Short.MAX_VALUE;
          total += val;
        }
        byte[] ns = shortToByteArray(
          (short) ((float) Short.MAX_VALUE *
            (total / (float) audioInfo.channel()))
        );
    ...
    private static ByteBuffer getBuffer(byte[] array) {
      ByteBuffer audioBuffer2 = BufferUtils.createByteBuffer(
        array.length
      );
  ...
  private static record MiddleLoadInput(
    ReadWait readWaiter,
    AudioInfo audioInfo
  ) {}

  private static record MiddleLoadResult(
    ReadWait readWaiter,
