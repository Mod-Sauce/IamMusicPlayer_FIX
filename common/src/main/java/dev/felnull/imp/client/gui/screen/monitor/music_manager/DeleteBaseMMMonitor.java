...
public abstract class DeleteBaseMMMonitor
  extends MusicManagerMonitor
{

  private static final Component CANCEL_TEXT = Component.translatable(
    "gui.cancel"
  );
  ...
  public void init(int leftPos, int topPos) {
    ...
    this.WARING = Component.translatable(
      "imp.text.deleteWarning",
      getWaringName()
    );
  ...
  public void render(
    ...
    float sc = Math.min(
      1f,
      (width - 20f) / (float) mc.font.width(WARING)
    );
    OERenderUtils.poseScaleAll(poseStack, sc);
    drawSmartCenterText(
      guiGraphics,
      WARING,
      (getStartX() + (float) width / 2f) / sc,
      (getStartY() + (float) height / 2f - 16.5f) / sc
    );
    poseStack.popPose();
  }
  ...
  public void renderAppearance(
    ...
    var wrtx = Component.translatable(
      "imp.text.deleteWarning",
      getWaringName(blockEntity)
    );
    float sc = Math.min(
      1f,
      (width - 20f) / (float) mc.font.width(wrtx)
    );
  ...
  @NotNull
  public abstract String getWaringName(
    MusicManagerBlockEntity musicManagerBlockEntity
  );
}
