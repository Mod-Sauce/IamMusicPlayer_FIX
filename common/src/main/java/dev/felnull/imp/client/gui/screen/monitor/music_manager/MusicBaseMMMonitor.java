  ...
  extends ImageNameBaseMMMonitor
{
  ...
  private static final ResourceLocation ADD_MUSIC_TEXTURE =
    new ResourceLocation(
      IamMusicPlayer.MODID,
      "textures/gui/container/music_manager/monitor/add_music.png"
    );
  private static final ResourceLocation SHOW_MUSIC_TEXTURE =
    new ResourceLocation(
      IamMusicPlayer.MODID,
      "textures/gui/container/music_manager/monitor/show_music.png"
    );
  private static final Component PLAYBACK_CONTROL_TEXT =
    Component.translatable("imp.button.playbackControl");
  protected static final Component MUSIC_SOURCE_TEXT =
    Component.translatable("imp.text.musicSource");
  private static final Component PLAYBACK_NON_PROGRESS_TEXT =
    Component.translatable("--:--/--:--");
  private static final Component PLAYBACK_LOADING_PROGRESS_TEXT =
    Component.translatable("imp.text.playbackLoading");
  ...
  public void init(int leftPos, int topPos) {
    ...
    this.playControlButton = this.addRenderWidget(
      new SmartButton(
        getStartX() + this.playBackX,
        getStartY() + this.playBackY,
        20,
        20,
        PLAYBACK_CONTROL_TEXT,
        n -> {
          if (getScreen().isMusicPlaying()) {
            getScreen().stopMusic();
          } else {
            if (!getMusicSource().isEmpty()) getScreen().playMusic(
              getMusicSource(),
              0
            );
          }
    ...
    this.addRenderWidget(
      new PlaybackProgressBar(
        getStartX() + this.playBackX + 22,
        getStartY() + this.playBackY + 14,
        Component.translatable("imp.progressBar.playbackControl"),
        () -> {
          if (getScreen().isMusicPlaying()) return getScreen()
            .getMusicPlayer()
    ...
    if (!getMusicAuthor().isEmpty()) setMusicAuthor(getMusicAuthor());
  }
  ...
  public void tick() {
    ...
    if (
      getScreen().isMusicPlaying() || getScreen().isMusicLoading()
    ) {
  ...
  public void render(
    ...
    OERenderUtils.drawTexture(
      isLoaderSelect ? ADD_MUSIC_TEXTURE : SHOW_MUSIC_TEXTURE,
      guiGraphics.pose(),
      getStartX(),
      getStartY(),
      0f,
    ...
    drawSmartText(
      guiGraphics,
      pt,
      getStartX() + playBackX + 22,
      getStartY() + playBackY + 4
    );

    if (AUTHOR_TEXT != null) drawSmartText(
      guiGraphics,
      AUTHOR_TEXT,
      getStartX() + this.playBackX,
      getStartY() + this.playBackY + 25
    );
  ...
  public void renderAppearance(
    ...
    if (!getMusicSource(blockEntity).isEmpty()) {
      pt = Component.literal(
        FNStringUtil.getTimeProgress(
          0,
          getMusicSource(blockEntity).getDuration()
        )
      );
    } else {
      pt = PLAYBACK_NON_PROGRESS_TEXT;
    ...
    OERenderUtils.renderTextureSprite(
      ...
      poseStack,
      multiBufferSource,
      onPxW * (this.playBackX + 22),
      monitorHeight - onPxH * (this.playBackY + 14 + 3),
      OERenderUtils.MIN_BREADTH * 4,
      0,
      0,
      0,
    ...
    if (!getMusicAuthor(blockEntity).isEmpty()) {
      int le =
        176 -
        mc.font.width(
          Component.translatable("imp.text.musicAuthor", "")
        );
      var aut = OEClientUtils.getWidthOmitText(
        getMusicAuthor(blockEntity),
        le,
        "..."
      );
      renderSmartTextSprite(
        poseStack,
        multiBufferSource,
        Component.translatable("imp.text.musicAuthor", aut),
        this.playBackX,
        this.playBackY + 25,
        OERenderUtils.MIN_BREADTH * 4,
        onPxW,
  ...
  protected void setMusicAuthor(String author) {
    if (author.isEmpty()) {
      ...
      AUTHOR_TEXT = Component.translatable(
        "imp.text.musicAuthor",
        aut
      );
    }
  }
  ...
  @NotNull
  protected String getMusicSourceName(
    @NotNull MusicManagerBlockEntity blockEntity
  ) {
  ...
  protected MusicMedia getRawMusicLoaderType(
    ...
    return IMPMusicMedias.getAllMedia().get(
      getMusicLoaderType(musicManagerBlockEntity)
    );
  }
  ...
  protected String getMusicLoaderType(
    ...
    return blockEntity.getMusicLoaderType(mc.player).isEmpty()
      ? "auto"
      : blockEntity.getMusicLoaderType(mc.player);
  }
}
