 ...
 */
public record MusicInstantaneous(
  long sampleTime,
  List<ChannelInstantaneous> channelInstantaneous
) {
  public static MusicInstantaneous create(
    long time,
    byte[] data,
    int channel,
    int bit
    ...
    for (int i = 0; i < channel; i++) cis.add(
      new ChannelInstantaneous(new float[60])
    );

    if (bit != 16) throw new RuntimeException("Unsupported bit");
