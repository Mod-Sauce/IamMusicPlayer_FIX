...
public class PlaybackProgressBar extends AbstractButton {
  ...
  public PlaybackProgressBar(
    ...
    Consumer<Float> playbackProgressControl
  ) {
  ...
  public void onClick(double d, double e) {
    ...
    if (isHoveredOrFocused()) {
      if (
        ...
        d >= getX() &&
        d <= (getX() + getWidth())
      ) playbackProgressControl.accept(
        (float) ((d - getX()) / getWidth())
      );
    }
  }
  ...
  public void renderWidget(
    ...
    OERenderUtils.drawTexture(
      ...
      getY(),
      52,
      48 + (isHoveredOrFocused() ? 3 : 0),
      (float) getWidth() * progressGetter.get(),
      getHeight()
    );
  }
