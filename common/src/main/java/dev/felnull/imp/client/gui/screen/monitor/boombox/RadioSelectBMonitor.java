...
public class RadioSelectBMonitor extends BoomboxMonitor {
  ...
  private static final ResourceLocation RADIO_SELECT_BG_TEXTURE =
    new ResourceLocation(
      IamMusicPlayer.MODID,
      "textures/gui/container/boombox/monitor/radio_select.png"
    );
  private static final ResourceLocation RADIO_SELECT_IMAGE_BG_TEXTURE =
    new ResourceLocation(
      IamMusicPlayer.MODID,
      "textures/gui/container/boombox/monitor/radio_select_image.png"
    );
  private static final Component RADIO_CHECKING_TEXT =
    Component.translatable("imp.text.radioChecking");
  private static final Component ENTER_STREAM_TEXT =
    Component.translatable("imp.text.enterStream");
  ...
  public RadioSelectBMonitor(
    BoomboxData.MonitorType monitorType,
    BoomboxScreen screen
  ) {
  ...
  public void init(int leftPos, int topPos) {
    ...
    this.radioUrlEditBox = addRenderWidget(
      new EditBox(
        IIMPSmartRender.mc.font,
        getStartX() + 2,
        getStartY() + height - 2 - 12,
        width - 4 - 35,
        12,
        Component.translatable("imp.editBox.radioUrl")
      )
    );
    ...
    this.radioStreamStartButton = this.addRenderWidget(
      new SmartButton(
        getStartX() + width - 34 - 1,
        getStartY() + height - 1 - 14,
        34,
        14,
        Component.translatable("imp.button.radioStreamStart"),
        n -> {
    ...
    this.radioStreamStartButton.setIcon(
  ...
  public void render(
    ...
    if (isRadioChecking()) {
      drawSmartText(
        ...
        RADIO_CHECKING_TEXT,
        getStartX() + 2,
        getStartY() + (height - 1f - 14f - 6.5f) / 2f
      );
    } else if (!getRadioSource().isEmpty()) {
      float st = 2;
      if (!getRadioImage().isEmpty()) {
        OERenderUtils.drawTexture(
          RADIO_SELECT_IMAGE_BG_TEXTURE,
          guiGraphics.pose(),
          getStartX(),
          getStartY(),
          0f,
      ...
      if (
        !getScreen().getBoomBoxData().getRadioName().isEmpty()
      ) drawSmartText(
        guiGraphics,
        Component.literal(
          OEClientUtils.getWidthOmitText(
            getScreen().getBoomBoxData().getRadioName(),
            width - 3 - st,
            "..."
          )
        ),
      ...
      if (!getScreen().getBoomBoxData().getRadioAuthor().isEmpty()) {
        ...
        drawSmartText(
          ...
          Component.translatable(
            ...
            OEClientUtils.getWidthOmitText(
              getScreen().getBoomBoxData().getRadioAuthor(),
              width - 3 - st - mc.font.width(tx),
              "..."
            )
          ),
          getStartX() + st,
  ...
  public void renderAppearance(
    ...
    if (!getRadioSource(data).isEmpty()) {
      ...
      if (!data.getRadioName().isEmpty()) renderSmartTextSprite(
        poseStack,
        multiBufferSource,
        Component.literal(
          OEClientUtils.getWidthOmitText(
  ...
  private boolean isRadioChecking() {
    return (
      this.radioCheckLoader != null && this.radioCheckLoader.isAlive()
    );
  }
    ...
    public void run() {
      ...
      try {
        ...
        if (otrack.isPresent()) {
          ...
          if (
            track.getSourceManager() instanceof
              YoutubeAudioSourceManager
          ) mc.submit(() ->
            setRadioImage(
              new ImageInfo(
                ImageInfo.ImageType.YOUTUBE_THUMBNAIL,
                info.identifier
              )
            )
          );
