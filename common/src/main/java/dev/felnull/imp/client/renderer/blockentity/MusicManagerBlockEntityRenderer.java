...
public class MusicManagerBlockEntityRenderer
  extends AbstractBlockEntityRenderer<MusicManagerBlockEntity>
{
  ...
  protected MusicManagerBlockEntityRenderer(
  ...
  public static void renderMusicManager(
    MusicManagerBlockEntity blockEntity,
    PoseStack poseStack,
    MultiBufferSource multiBufferSource,
    int i,
    ...
    if (blockEntity.isPowered() && random.nextBoolean()) acLmp =
      IMPModels.MUSIC_MANAGER_ACCESS_LAMP.get();

    poseStack.pushPose();
    OERenderUtils.poseRotateDirection(
      poseStack,
      blockEntity.getBlockState().getValue(MusicManagerBlock.FACING),
      1
    );
  ...
  private static MusicManagerMonitor getMonitor(
