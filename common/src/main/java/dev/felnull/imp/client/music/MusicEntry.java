...
public class MusicEntry {
  ...
  private final Map<UUID, MusicTracker> reserveTrackers =
    new HashMap<>();
  ...
  public float getCurrentPositionProgress() {
    ...
    return (
      (float) getCurrentPosition() / (float) source.getDuration()
    );
  }
  ...
  protected void destroy() {
    stopped = true;
    if (musicPlayer.get() != null) musicPlayer
      .get()
      .destroyNonThrow();
  }
  ...
  protected boolean updateMusicTracker(
    UUID speakerId,
  ...
  private MusicLoader selectLoader(MusicSource source) {
    List<MusicLoader> loaders = IMPMusicLoaders.getAllLoader()
      ...
      .map(Supplier::get)
      .sorted(
        Comparator.comparingInt(MusicLoader::priority).reversed()
      )
      .toList();
  ...
  private static record LoadResult(
    boolean success,
    Throwable error
  ) {}
