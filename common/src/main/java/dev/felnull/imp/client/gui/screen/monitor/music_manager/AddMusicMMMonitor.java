...
public class AddMusicMMMonitor extends SavedMusicBaseMMMonitor {
  ...
  private static final Component MUSIC_CHECKING_TEXT =
    Component.translatable("imp.text.musicChecking");
  private static final Component MUSIC_GUESSING_TEXT =
    Component.translatable("imp.text.musicGuessing");
  private static final Component MUSIC_LOAD_FAILURE_TEXT =
    Component.translatable("imp.text.loadFailure");
  private static final Component AUTO_ENTER_TEXT =
    Component.translatable("imp.text.enterText.auto");
  private static final Component AUTO_INFO_TEXT =
    Component.translatable("imp.text.loaderTypeInfo.auto");
  private static final Component AUTO_FAILURE_TEXT =
    Component.translatable("imp.text.loadFailure.auto");
  private static final Component UPLOAD_MUSIC_TEXT =
    Component.translatable("imp.button.uploadMusic");
  protected static final Component IMPORT_TEXT =
    Component.translatable("imp.button.import");
  ...
  public void init(int leftPos, int topPos) {
    ...
    List<String> medias = new ArrayList<>(
      IMPMusicMedias.getAllMedia().keySet()
    );
    Collections.reverse(medias);
    this.musicLoaderTypes.addAll(medias);

    this.musicLoaderTypesFixedButtonsList = this.addRenderWidget(
      new MusicLoaderTypesFixedListWidget(
        getStartX() + 189,
        getStartY() + 23,
        175,
        65,
        Component.translatable("imp.fixedList.musicLoaderTypes"),
        5,
        musicLoaderTypes,
        (widget, item) -> {
        ...
        this.musicLoaderTypesFixedButtonsList,
        n -> getMusicLoaderType().equals(n)
      )
    );

    this.musicSourceNameEditBox = new EditBox(
      IIMPSmartRender.mc.font,
      getStartX() + 189,
      getStartY() + 112,
      isMSNShortWidth() ? 141 : 177,
      12,
      Component.translatable("imp.editBox.musicSourceName")
    );
    ...
    this.musicSourceNameEditBox.setResponder(
      this::setMusicSourceName
    );
    ...
    this.searchButton = this.addRenderWidget(
      new SmartButton(
        getStartX() + 331,
        getStartY() + 111,
        35,
        14,
        Component.translatable("imp.button.search"),
        n -> {
          insMonitor(
    ...
    this.uploadButton = this.addRenderWidget(
      new SmartButton(
        getStartX() + 189,
        getStartY() + 111,
        177,
  ...
  public void render(
    ...
    if (
      musicLoadThread != null && !getMusicSourceName().isEmpty()
    ) drawSmartText(
      guiGraphics,
      "auto".equals(getMusicLoaderType())
        ? MUSIC_GUESSING_TEXT
        : MUSIC_CHECKING_TEXT,
      getStartX() + 189,
      getStartY() + 128
    ...
    if (getRawMusicLoaderType() != null) {
      drawSmartText(
        guiGraphics,
        getRawMusicLoaderType().getEnterText(),
        getStartX() + 189,
        getStartY() + 102
      );
    ...
    if (loadFailure && !getMusicSourceName().isEmpty()) drawSmartText(
      guiGraphics,
      "auto".equals(getMusicLoaderType())
        ? AUTO_FAILURE_TEXT
        : MUSIC_LOAD_FAILURE_TEXT,
      getStartX() + 189,
      getStartY() + 128
    );
  }
  ...
  public void renderAppearance(
    ...
    if (!"upload".equals(getMusicLoaderType(blockEntity))) {
      ...
      renderSmartEditBoxSprite(
        ...
        112,
        OERenderUtils.MIN_BREADTH * 4,
        (rtp != null && rtp.isSearchable()) ? 141 : 177,
        12,
        i,
        j,
        onPxW,
      ...
      if (rtp != null && rtp.isSearchable()) renderSmartButtonSprite(
        poseStack,
        multiBufferSource,
        333,
        111,
    ...
    for (int k = 0; k < Math.min(5, musicLoaderTypes.size()); k++) {
      ...
      renderSmartButtonBoxSprite(
        ...
        onPxH,
        monitorHeight,
        lt.equals(getMusicLoaderType(blockEntity))
      );
      ...
      if (
        ...
        var icon =
          type != null
            ? type.getIcon()
            : MusicLoaderTypesFixedListWidget.UPLOAD_ICON;
        OERenderUtils.renderTextureSprite(
          ...
          (189f + 1f) * onPxW,
          monitorHeight -
            (23f + ((float) k * 13f) + 1f + 11f) * onPxH,
          OERenderUtils.MIN_BREADTH * 5,
          0,
          0,
          0,
          11f * onPxW,
      ...
      renderSmartTextSprite(
        ...
        multiBufferSource,
        type == null
          ? Component.translatable("imp.loaderType." + lt)
          : type.getMediaName(),
        tx,
        23 + (k * 13) + (13f - 6.5f) / 2f + 1f,
        OERenderUtils.MIN_BREADTH * 5,
        onPxW,
        onPxH,
        monitorHeight,
  ...
  public boolean done(ImageInfo imageInfo, String name) {
    ...
    if (
      ...
      musicManagerBlock.getSelectedPlayList(mc.player) != null
    ) NetworkManager.sendToServer(
      IMPPackets.MUSIC_ADD,
      new IMPPackets.MusicMessage(
        musicManagerBlock.getSelectedPlayList(mc.player),
        name,
        getMusicAuthor(),
        imageInfo,
        ms,
  ...
  public boolean canDone(MusicManagerBlockEntity blockEntity) {
    return (
      ...
      !getMusicSource(blockEntity).isEmpty()
    );
  }
  ...
  public List<Component> getNotEntered(
    ...
    if (getMusicSource(blockEntity).isEmpty()) names.add(
      MUSIC_SOURCE_TEXT
    );
  ...
  public void tick() {
    super.tick();
    this.musicSourceNameEditBox.visible = isMSNVisible();
    this.musicSourceNameEditBox.setWidth(
      isMSNShortWidth() ? 141 : 177
    );
    ...
    if (
      (getScreen().isMusicPlaying() &&
        !getMusicSource().equals(
          getScreen().getMusicPlayer().getSource()
        )) ||
      (getScreen().isMusicLoading() &&
        !getMusicSource().equals(
          getScreen().getMusicPlayer().getSource()
        ))
    ) getScreen().stopMusic();
  ...
  private void setMusicSourceName(String name) {
    if (!getMusicSourceName().equals(name)) startMusicLoad(
      name,
      true
    );
  ...
  private void setLoadResult(
    MusicMediaResult result,
    boolean autoIn
  ) {
    if (result != null) {
      setMusicSource(result.source(), result.author());
      if (autoIn) {
        if (result.imageInfo() != null) setImage(result.imageInfo());
        if (result.name() != null) this.nameEditBox.setValue(
          result.name()
        );
      }
    } else {
  ...
  private void startMusicLoad(String name, boolean autoIn) {
    stopMusicLoad();
    if (
      getRawMusicLoaderType() != null ||
      "auto".equals(getMusicLoaderType()) ||
      "upload".equals(getMusicLoaderType())
    ) {
      this.loadFailure = false;
      this.musicLoadThread = new MusicLoadThread(
        getMusicLoaderType(),
        name,
        autoIn
      );
  ...
  private class MusicLoadThread extends FlagThread {
    ...
    public void run() {
      try {
        ...
        if ("upload".equals(loaderType)) loader = IMPMusicMedias.HTTP;
        if (isStopped()) return;
        ...
        if (loader != null) {
          ...
          if (ar != null) {
            ...
            IIMPSmartRender.mc.submit(() -> {
              musicSourceNameEditBox.setValue(
                ar.getValue().source().getIdentifier()
              );
              startMusicLoad(
                ar.getValue().source().getIdentifier(),
                autoIn
