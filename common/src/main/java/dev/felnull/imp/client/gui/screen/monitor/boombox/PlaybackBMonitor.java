...
public class PlaybackBMonitor extends PlayBackFiniteBaseBMMonitor {
  ...
  private static final Component NO_ANTENNA_TEXT =
    Component.translatable("imp.text.noAntenna");
  ...
  protected boolean canPlay(BoomboxData data) {
    return (
      IMPItemUtil.isCassetteTape(data.getCassetteTape()) &&
      getMusic(data) != null
    );
  }
  ...
  public void render(
    ...
    if (
      ...
      if (music == null) drawSmartCenterText(
        guiGraphics,
    ...
    long noAntennaTime =
      System.currentTimeMillis() - getScreen().lastNoAntenna;
    if (noAntennaTime <= 3000) {
      float alpha = Math.min(
        3f - ((float) noAntennaTime / 3000f) * 3f,
        1f
      );
  ...
  public void renderAppearance(
    ...
    if (
      !cassetteTape.isEmpty() &&
      IMPItemUtil.isCassetteTape(cassetteTape)
    ) {
      Music music = CassetteTapeItem.getMusic(cassetteTape);
      if (music == null) renderSmartCenterTextSprite(
        poseStack,
        multiBufferSource,
        NO_MUSIC_CASSETTE_TAPE_TEXT,
        ((float) width / 2f),
