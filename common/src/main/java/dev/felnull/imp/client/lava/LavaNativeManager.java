...
public class LavaNativeManager {
  ...
  public boolean load(String osAndArch, String name) {
    Path nativesDir =
      LavaPlayerLoader.getNaiveLibraryFolder().resolve(osAndArch);
   ...
   */
  public CompletableFuture<Boolean> downloadNatives(
    String osAndArch
  ) {
  ...
  private boolean downloadAndExtractNatives(String osAndArch)
    ...
    JsonObject versionJson = manifestJson.getAsJsonObject(
      NATIVES_VERSION
    );
  ...
  private JsonObject downloadManifest() throws Exception {
    ...
    LOGGER.info(
      "Downloading natives manifest from: {}",
      manifestUrlString
    );
   ...
   */
  private Path downloadWithRetry(URL url, Path targetDir)
    throws Exception {
   ...
   */
  private void downloadFile(URL url, Path destination)
    throws IOException {
  ...
  private boolean validateNativesIntegrity(File directory) {
    try {
      ...
      if (nativeLibs.isEmpty()) {
        LOGGER.error(
          "No native library files found in {}",
          directory
        );
        return false;
      }
   ...
   */
  private List<File> filterNativeLibrariesForCurrentOS(
    List<File> files
  ) {
    ...
    return files
      ...
      .filter(f -> {
        ...
        if (
          os.contains("mac") && name.endsWith(".dylib")
        ) return true;
