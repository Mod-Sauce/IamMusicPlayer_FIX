...
public class LavaNativeManager {
    ...
    public boolean load(String osAndArch, String name) {
        var npF =
                LavaPlayerLoader.getNaiveLibraryFolder().resolve(osAndArch).toFile();
        if (!checked(npF)) {
            try {
    ...
    private void downloadNatives(String osAndArch) throws Exception {
        ...
        try (BufferedReader reader =
                     new BufferedReader(new InputStreamReader(FNURLUtil.getStream(
                             new URL(IamMusicPlayer.getConfig().lavaPlayerNativesURL))))) {
        ...
        FNDataUtil.readZipStreamed(new BufferedInputStream(FNURLUtil.getStream(
                        new URL(jooo.get("url").getAsString()))),
        ...
        if (fls.size() == 1 && !jo.get("hash").isJsonObject()) {
            try {
                ...
                var th = new String(Hex.encodeHex(
                        FNDataUtil.createMD5Hash(Files.readAllBytes(fls.get(0).toPath()))));
                return th.equals(ch);
            } catch (Exception ignored) {
            }
        } else if (jo.get("hash").isJsonObject()
                && jo.get("hash").getAsJsonObject().size() == fls.size()) {
