...
public abstract class PlayBackBaseBMonitor extends BoomboxMonitor {
  ...
  protected static final ResourceLocation PLAYING_BG_TEXTURE =
    new ResourceLocation(
      IamMusicPlayer.MODID,
      "textures/gui/container/boombox/monitor/playing.png"
    );
  protected static final ResourceLocation PLAYING_IMAGE_TEXTURE =
    new ResourceLocation(
      IamMusicPlayer.MODID,
      "textures/gui/container/boombox/monitor/playing_image.png"
    );
  private VolumeWidget volumeWidget;
  private PlayBackControlWidget playBackControlWidget;

  public PlayBackBaseBMonitor(
  ...
  public void init(int leftPos, int topPos) {
    ...
    this.playBackControlWidget = this.addRenderWidget(
      new PlayBackControlWidget(
        getStartX() + (isShortProgressBar() ? 38 : 2),
        getStartY() + 25,
        () ->
          getScreen().isPlaying()
  ...
  public void render(
    ...
    if (!getPlayBackImage().isEmpty()) OERenderUtils.drawTexture(
      PLAYING_IMAGE_TEXTURE,
      guiGraphics.pose(),
      getStartX(),
      getStartY(),
      0f,
  ...
  public void renderAppearance(
    ...
    int sx = 2;
    if (!getPlayBackImage(data).isEmpty()) {
      getPlayImageRenderer().renderSprite(
        getPlayBackImage(data),
        poseStack,
        multiBufferSource,
        1 * onPxW,
        monitorHeight - (1 + height - 2) * onPxH,
  ...
  @NotNull
  protected abstract ImageInfo getPlayBackImage(BoomboxData data);
  ...
  @NotNull
  protected abstract String getPlayBackName(BoomboxData data);
  ...
  @NotNull
  protected abstract String getPlayBackAuthor(BoomboxData data);
  ...
  @NotNull
  protected abstract MusicSource getPlayBackSource(BoomboxData data);
