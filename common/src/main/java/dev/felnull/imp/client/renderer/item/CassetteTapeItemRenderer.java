...
public class CassetteTapeItemRenderer implements BEWLItemRenderer {
  ...
  public void render(
    ...
    VertexConsumer ivb = ItemRenderer.getFoilBufferDirect(
      multiBufferSource,
  ...
  private static void renderMusicInfo(
    PoseStack poseStack,
    VertexConsumer ivb,
    MultiBufferSource multiBufferSource,
    ItemStack stack,
    ...
    BakedModel labelModel = music.getImage().isEmpty()
      ? IMPModels.CASSETTE_TAPE_LABEL_NO_IMAGE_MODEL.get()
      : IMPModels.CASSETTE_TAPE_LABEL_MODEL.get();
    poseStack.pushPose();
    OERenderUtils.poseTrans16(poseStack, 3d, 1d, 4d);
    OERenderUtils.renderModel(poseStack, ivb, labelModel, i, i1);
    poseStack.translate(
      0,
      (1f / 16f) * 0.025f + OERenderUtils.MIN_BREADTH,
      0
    );
  ...
  private static void renderBase(
    ...
    if (stack.getItem() instanceof CassetteTapeItem) {
      CassetteTapeItem.BaseType type = (
        (CassetteTapeItem) stack.getItem()
      ).getType();
      ModelHolder holder = IMPModels.CASSETTE_TAPE_BASE_NORMAL_MODEL;
      int color = ((CassetteTapeItem) stack.getItem()).hasCustomColor(
        stack
      )
        ? ((CassetteTapeItem) stack.getItem()).getColor(stack)
        : 0x1a1a1a;
      if (type == CassetteTapeItem.BaseType.GLASS) {
        holder = ((CassetteTapeItem) stack.getItem()).hasCustomColor(
          stack
        )
          ? IMPModels.CASSETTE_TAPE_BASE_GLASS_COLOR_MODEL
          : IMPModels.CASSETTE_TAPE_BASE_GLASS_MODEL;
        ivb = ItemRenderer.getFoilBufferDirect(
          multiBufferSource,
  ...
  private static void renderTapeConecter(
    PoseStack poseStack,
    float angle,
    VertexConsumer ivb,
    double x,
    ...
    BakedModel tapeConecterModel =
      IMPModels.CASSETTE_TAPE_CONECTER.get();
    poseStack.pushPose();
  ...
  private static void renderTapeRoll(
    PoseStack poseStack,
    float par,
    float roll,
    VertexConsumer ivb,
    ...
    BakedModel tapeCoreModel =
      IMPModels.CASSETTE_TAPE_CORE_MODEL.get();
    BakedModel tapeCoreAroundModel =
      IMPModels.CASSETTE_TAPE_CORE_AROUND_MODEL.get();
    BakedModel tapeRollModel =
      IMPModels.CASSETTE_TAPE_ROLL_MODEL.get();
