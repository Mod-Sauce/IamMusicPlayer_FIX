  ...
  implements IIMPSmartRender
{
  ...
  public VolumeWidget(
    ...
    int y,
    IntValue volume,
    BooleanSupplier mute,
    BooleanConsumer setMute
  ) {
  ...
  public void renderWidget(
    ...
    if (imute) OERenderUtils.drawTexture(
      MusicManagerMonitor.WIDGETS_TEXTURE,
      guiGraphics.pose(),
      getX() + 3,
      getY(),
    ...
    drawSmartText(
      guiGraphics,
      Component.literal(String.valueOf(vol)),
      getX() + 5 + lfs,
      getY() + 0.5f,
  ...
  public void onClick(double d, double e) {
    super.onClick(d, e);
    if (setMute != null) setMute.accept(!mute.getAsBoolean());
  }

  @Override
  public boolean mouseScrolled(double d, double e, double f) {
    float mv = 1;
    if (OEClientUtils.isKeyInput(mc.options.keyShift)) mv *= 10;
    if (OEClientUtils.isKeyInput(mc.options.keySprint)) mv *= 10;
    int an = (int) (mv * f);
