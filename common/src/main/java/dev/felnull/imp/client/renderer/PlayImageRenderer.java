...
public class PlayImageRenderer {
  ...
  private static final ResourceLocation MISSING_YOUTUBE_THUMBNAIL_TEXTURE =
    new ResourceLocation(
      IamMusicPlayer.MODID,
      "textures/gui/image/missing_youtube_thumbnail.png"
    );
  private static final ResourceLocation MISSING_SOUND_CLOUD_THUMBNAIL_TEXTURE =
    new ResourceLocation(
      IamMusicPlayer.MODID,
      "textures/gui/image/missing_sound_cloud_artwork.png"
    );
  private static final PlayImageRenderer INSTANCE =
    new PlayImageRenderer();
  private static final String YOUTUBE_THUMBNAIL_URL =
    "https://i.ytimg.com/vi/%s/hqdefault.jpg";
  private static final Pattern YOUTUBE_THUMBNAIL_URL_REGEX =
    Pattern.compile("https://i.ytimg.com/vi/.+/hqdefault.jpg");
  private static final Pattern SOUND_CLOUD_URL_REGEX =
    Pattern.compile("https://soundcloud.com/.+");
  ...
  public String getSwapURL(String url) {
    if (url.startsWith("imp_sct_")) {
      ...
      if (SOUND_CLOUD_URL_REGEX.matcher(scurl).matches()) {
        String ret = null;
        try {
          ret = extractSoundCloudImage(
            FNURLUtil.getResponse(new URI(scurl).toURL())
          );
  ...
  public boolean isAllowURL(String url) {
    if (
      YOUTUBE_THUMBNAIL_URL_REGEX.matcher(url).matches()
    ) return true;
    if (url.startsWith("imp_sct_")) {
      var scurl = url.substring("imp_sct_".length());
  ...
  public void drawPlayerFaceImage(
    String name,
    PoseStack poseStack,
    float x,
    float y,
  ...
  public void renderSprite(
    ...
    var td = getImageTexture(imageInfo, cash);
    if (td != null) renderTextureImageSprite(
      td.getLeft(),
      poseStack,
      multiBufferSource,
      td.getRight().x,
  ...
  private void renderPlayerFaceImageSprite(
    String name,
    PoseStack poseStack,
    MultiBufferSource multiBufferSource,
    float x,
  ...
  private void renderTextureImageSprite(
    ResourceLocation location,
    PoseStack poseStack,
    MultiBufferSource multiBufferSource,
    float wScale,
  ...
  private Pair<ResourceLocation, Vec2> getImageTexture(
    ImageInfo imageInfo,
    boolean cash
  ) {
    if (imageInfo.getImageType() == ImageInfo.ImageType.URL) {
      var loc = OETextureUtils.getAndLoadURLTextureAsync(
        imageInfo.getIdentifier(),
        cash
      ).of();
      ...
      var loc = OETextureUtils.getAndLoadURLTextureAsync(
        ...
        cash
      ).of(MISSING_YOUTUBE_THUMBNAIL_TEXTURE);
      var scale = OETextureUtils.getTextureScale(loc);
      float w = 1;
      float h = 1;
      ...
      imageInfo.getImageType() ==
      ImageInfo.ImageType.SOUND_CLOUD_ARTWORK
    ) {
      var idf = imageInfo.getIdentifier();
      var loc = OETextureUtils.getAndLoadURLTextureAsync(
        "imp_sct_" + idf,
        cash
      ).of(MISSING_SOUND_CLOUD_THUMBNAIL_TEXTURE);
      var scale = OETextureUtils.getTextureScale(loc);
      float w = 1;
      float h = 1;
  ...
  private static String extractSoundCloudImage(String html) {
    var ts = html.substring(
      html.indexOf(SCT_IMG_ST) + SCT_IMG_ST.length()
    );
