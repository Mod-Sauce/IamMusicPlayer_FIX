...
public class IMPRHash {
    ...
    private boolean Linux(Type os, String arch) {
        ...
        String expectedHashRaw = HashUnix(os.toString(), normalizedArch, true);
    ...
    private boolean Windows(Type os, String arch) {
        ...
        LOGGER.info("Expected hash (connector): {}", expectedHash_connector);
        LOGGER.info("Actual hash (connector):   {}", actualHash_connector);

        boolean libMatch = Objects.equals(expectedHash_lib, actualHash_lib);
        ...
        if (!connectorMatch) {
            LOGGER.warn("Hash mismatch detected for Windows connector file!");
            int minLength = Math.min(
            ...
            if (
                    expectedHash_connector.length() != actualHash_connector.length()
            ) {
                LOGGER.info(
                        "Length mismatch: expected {} but got {}",
                        expectedHash_connector.length(),
