...
public class MusicBuffer {
  ...
  private final List<MusicSpeaker> insertedSpeakers =
    new ArrayList<>();
  ...
  public MusicBuffer(
    ...
    alBufferData(
      this.buffer,
      getFormat(relative ? audioData.channel() : 1, audioData.bit()),
      buffer,
  ...
  public void release() {
    if (!canRelease()) throw new RuntimeException(
      "Cannot be released because it is in use"
    );
