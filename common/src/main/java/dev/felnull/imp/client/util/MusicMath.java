...
public class MusicMath {
  ...
  public static double calculateVolume(double playerVolume) {
    return Mth.clamp(
      playerVolume * IamMusicPlayer.getConfig().volume,
      0.0,
      1.0
    );
  }

  public static double calculatePseudoAttenuation(
    Vec3 position,
    float range,
    double volume
  ) {
    ...
    AL11.alGetListener3f(
      AL11.AL_POSITION,
      listenerX,
      listenerY,
      listenerZ
    );
    Vec3 listener = new Vec3(
      listenerX[0],
      listenerY[0],
      listenerZ[0]
    );
    return calculatePseudoAttenuation(
      (float) listener.distanceTo(position),
      range,
      volume
    );
  }

  private static double calculatePseudoAttenuation(
    float distance,
    float range,
    double volume
  ) {
