...
public class PlayListMMMonitor extends MusicManagerMonitor {
  ...
  private static final ResourceLocation PLAY_LIST_TEXTURE =
    new ResourceLocation(
      IamMusicPlayer.MODID,
      "textures/gui/container/music_manager/monitor/play_list.png"
    );
  private static final Component ADD_PLAYLIST_TEXT =
    Component.translatable("imp.button.addPlaylist");
  private static final Component ADD_MUSIC_TEXT =
    Component.translatable("imp.button.addMusic");
  private static final Component SORT_TYPE_NAME_TEXT =
    Component.translatable(
      "imp.sortType." + SortButton.SortType.NAME.getName()
    );
  private static final Component ORDER_TYPE_DESCENDING_TEXT =
    Component.translatable(
      "imp.orderType." + SortButton.OrderType.DESCENDING.getName()
    );
  private static final Component DETAIL_TEXT = Component.translatable(
    "imp.button.detailPlaylist"
  );
  ...
  public void init(int leftPos, int topPos) {
    ...
    this.myPlayListFixedButtonsList = addRenderWidget(
      new MyPlayListFixedListWidget(
        getStartX() + 1,
        getStartY() + 20,
        musicPlayLists,
        (widget, item) -> setSelectedPlayList(item.getUuid()),
        this.myPlayListFixedButtonsList,
        n -> n.getUuid().equals(getSelectedPlayList())
      )
    );

    this.addPlaylistButton = addRenderWidget(
      new SmartButton(
        getStartX() + 1,
        getStartY() + 189,
        72 + 9,
    ...
    this.addMusic.active =
      getSelectedMusicPlayList() != null &&
      getSelectedMusicPlayList()
        .getAuthority()
        .getAuthorityType(
    ...
    this.musicSortButton = addRenderWidget(
    ...
    this.musicOrderButton = addRenderWidget(
      new SortButton.OrderTypeButton(
        getStartX() + 271,
        getStartY() + 189,
        n -> updateMusics(),
        true,
    ...
    this.musicsFixedButtonsList = this.addRenderWidget(
      new MusicsFixedListWidget(
        getStartX() + 102,
        getStartY() + 40,
        267,
        148,
        Component.translatable("imp.fixedList.musics"),
        4,
        musics,
        (widget, item) -> {
        ...
        false,
        this.musicsFixedButtonsList
      )
    );

    this.detailButton = this.addRenderWidget(
      new SmartButton(
        getStartX() + 336,
        getStartY() + 20,
        33,
    ...
    this.playlistOrderButton = addRenderWidget(
      new SortButton.OrderTypeButton(
        getStartX() + 82 + 9,
        getStartY() + 189,
        n -> updateList(),
  ...
  public void render(
    ...
    if (INFO_TEXT != null) drawSmartText(
      guiGraphics,
      INFO_TEXT,
      getStartX() +
        width -
        IIMPSmartRender.mc.font.width(INFO_TEXT) -
        3,
      getStartY() + 11
    );
    ...
    if (pl != null) {
      ...
      var au = pl
        ...
        .entrySet()
        .stream()
        .sorted(Comparator.comparingInt(o -> o.getValue().getLevel()))
        .map(Map.Entry::getKey)
        .filter(n -> !n.equals(pl.getAuthority().getOwner()))
        .toList();
      if (au.size() == 0) {
        OERenderUtils.drawPlayerFace(
          poseStack,
          pl.getAuthority().getOwner(),
          getStartX() + 328,
          getStartY() + 21,
          7
        );
  ...
  public void renderAppearance(
    ...
    renderSmartButtonSprite(
      ...
      256,
      256,
      getSelectedPlayList(blockEntity) == null
    );
    ...
    if (pls != null) {
      ...
      for (int k = 0; k < Math.min(8, pls.size()); k++) {
        ...
        renderSmartButtonBoxSprite(
          ...
          onPxH,
          monitorHeight,
          playList.getUuid().equals(getSelectedPlayList(blockEntity))
        );
        float sx = 1;
        if (!playList.getImage().isEmpty()) {
          sx += 18 + 2;
          PlayImageRenderer.getInstance().renderSprite(
            playList.getImage(),
            poseStack,
            multiBufferSource,
            3 * onPxW,
            monitorHeight - (20 + (k * 21) + 2 + 17) * onPxH,
        ...
        renderSmartTextSprite(
          ...
          multiBufferSource,
          Component.literal(
            MyPlayListFixedListWidget.dateFormat.format(
              new Date(playList.getCreateDate())
            )
          ),
          sx + 3,
          20 + (k * 21) + 12,
    ...
    updateInfoText();
    if (INFO_TEXT != null) renderSmartTextSprite(
      poseStack,
      multiBufferSource,
      INFO_TEXT,
      width - IIMPSmartRender.mc.font.width(INFO_TEXT) - 3,
      11,
      OERenderUtils.MIN_BREADTH * 2,
      onPxW,
      onPxH,
    ...
    var ms = getSyncManager().getMusics(
      getSelectedPlayList(blockEntity)
    );
    int msc = 0;
    if (ms != null) {
      msc = ms.size();
      for (int k = 0; k < Math.min(4, ms.size()); k++) {
        ...
        if (!music.getImage().isEmpty()) {
          ...
          PlayImageRenderer.getInstance().renderSprite(
            ...
            multiBufferSource,
            103 * onPxW,
            monitorHeight - (40 + (k * 37) + 1 + 35) * onPxH,
            OERenderUtils.MIN_BREADTH * 4,
            35 * onPxH,
            i,
            j
        ...
        renderSmartTextSprite(
          ...
          Math.min(
            1,
            (267f - 9f - sx - 15f) /
              (float) IIMPSmartRender.mc.font.width(nt)
          ),
          i
        );
        ...
        renderSmartTextSprite(
          poseStack,
          multiBufferSource,
          Component.literal(music.getAuthor()),
          sx + 3 + 102,
          40 + (k * 37) + 14,
          OERenderUtils.MIN_BREADTH * 4,
          onPxW,
        ...
        var pname = OEClientUtils.getPlayerNameByUUID(
          music.getOwner()
        )
          .map(n -> (Component) Component.literal(n))
          .orElse(MusicsFixedListWidget.UNKNOWN_PLAYER_TEXT);

        poseStack.pushPose();
        poseStack.translate(
          (sx + 102 + 2) * onPxW,
          monitorHeight - (40 + (k * 37) + 23 + 9) * onPxH,
          OERenderUtils.MIN_BREADTH * 4
        );
  ...
  private void updateList() {
    ...
    if (musicPlayListsCash != null) musicPlayLists.addAll(
      playlistSortButton.sort(musicPlayListsCash, playlistOrderButton)
    );
  ...
  private void updateMusics() {
    musics.clear();
    if (musicsCash != null) musics.addAll(
      musicSortButton.sort(musicsCash, musicOrderButton)
    );
  }
  ...
  private void updateInfoText() {
    ...
    if (pls.getMyPlayListInfo() != lastPlayListInfo) {
      ...
      if (lastPlayListInfo != null) {
        INFO_TEXT = Component.translatable(
          "imp.text.playlistInfo",
          lastPlayListInfo.playListCount(),
          lastPlayListInfo.musicCount()
        );
      } else {
        INFO_TEXT = null;
      }
  ...
  public MusicPlayList getSelectedMusicPlayList() {
    return musicPlayLists
      .stream()
      .filter(n -> n.getUuid().equals(getSelectedPlayList()))
      .findFirst()
      .orElse(null);
  }
