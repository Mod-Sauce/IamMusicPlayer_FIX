...
public class MusicLinesOverlay {
  ...
  public void render(GuiGraphics guiGraphics, float tickDelta) {
    ...
    int ct = 0;
    for (Map.Entry<UUID, MusicEntry> entry : me
      .getMusicEntries()
      .entrySet()) {
      drawMusicLine(
        guiGraphics,
        1,
        fh + 3 + (ct * (65 + mc.font.lineHeight)),
        entry.getKey(),
        entry.getValue()
      );
      ct++;
  ...
  private void drawMusicLine(
    ...
    String duStr = source.isLive()
      ? ("Live(" +
          FNStringUtil.getTimeFormat(entry.getCurrentPosition()) +
          ")")
    ...
    if (idf.length() > 15) idf = idf.substring(0, 15) + "...";
    ...
    drawText(
      guiGraphics,
      String.format("%s %s", mst, duStr) + text,
      x + 1,
      y
    );
    ...
    for (int i = 0; i < ssc; i++) {
      float ow =
        (((float) sw - 4f) - ((float) x + 2f)) / ((float) ssc - 1f);
      OERenderUtils.drawFill(
        poseStack,
        x + 2 + (ow * i),
        y + fh - 1 + 60,
    ...
    if (!source.isLive()) {
      for (MusicLoadChunk chunk : chunks) {
        ...
        drawLine(
          ...
          x,
          y,
          (float) chunk.position() / (float) source.getDuration(),
          ((float) chunk.duration() / (float) source.getDuration()) *
            (float) all,
          1f,
          l ? 0x7000FF00 : 0x7000C900
        );
      }
    } else {
      for (MusicLoadChunk chunk : chunks) {
        if (chunk.position() / liveLoop != loopCount) continue;
        boolean l = (chunk.position() / 1000) % 2 == 0;
        ...
        drawLine(
          ...
          x,
          y,
          (float) liveLoopChunkPos / (float) liveLoop,
          ((float) chunk.duration() / (float) liveLoop) * (float) all,
          1f,
          l ? 0x7000FF00 : 0x7000C900
        );
      }
    ...
    if (!source.isLive()) {
      drawPositionLine(
        ...
        x,
        y,
        (float) entry.getStartPosition() /
          (float) source.getDuration(),
        0xFF0000FF
      );
    } else {
      if (loopCount == 0) drawPositionLine(
        poseStack,
        x,
        y,
        (float) entry.getStartPosition() / liveLoop,
        0xFF0000FF
      );
    }
    ...
    int len = all / 10;
    float crunt =
      (float) entry.getCurrentPosition() /
      (float) source.getDuration();
