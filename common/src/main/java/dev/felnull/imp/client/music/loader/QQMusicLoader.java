...
public class QQMusicLoader extends LavaMusicLoader{
    @Override
    protected boolean isSupportMedia(MusicSource source) {
        return IMPMusicMedias.QQ_MUSIC.getName().equals(source.getLoaderType());
    }
    ...
    public void tryLoad(@NotNull MusicSource source) throws Exception {
        if(!IamMusicPlayer.getConfig().QQMusicConfig.enableQQMusic)return;
        super.tryLoad(source);
    }

    @Override
    protected String wrappedIdentifier(MusicSource source) {
        return "qq_music:" + source.getIdentifier();
    }
}
