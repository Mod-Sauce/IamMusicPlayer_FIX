  ...
  extends SavedPlayListBaseMMMonitor
{
  ...
  protected static final Component IMPORTING_TEXT =
    Component.translatable("imp.text.importing");
  protected static final Component IMPORT_FAILURE_TEXT =
    Component.translatable("imp.text.importFailure");
  private ImportMusicLoader importMusicLoader;
  private boolean failureImportPlayList;

  public CreatePlayListMMMonitor(
  ...
  public void render(
    ...
    if (isImporting()) {
      ...
      ipTx = Component.translatable(
        "imp.text.importMusicCount",
        getImportMusicsCount()
      );
    }
    if (ipTx != null) drawSmartText(
      guiGraphics,
      ipTx,
      getStartX() + width - 95 + 7,
      getStartY() + 184
    );
  }
  ...
  public void renderAppearance(
    ...
    if (getImportMusicsCount(blockEntity) > 0) renderSmartTextSprite(
      poseStack,
      multiBufferSource,
      Component.translatable(
        "imp.text.importMusicCount",
        getImportMusicsCount(blockEntity)
      ),
      width - 95 + 7,
      184,
  ...
  private void sendAddPacket(
    ImageInfo imageInfo,
    String name,
    PublishingType pubType,
    InitialAuthorityType initAuthType,
    List<UUID> invitePlayers,
    List<Music> importMusics
  ) {
    NetworkManager.sendToServer(
      IMPPackets.MUSIC_PLAYLIST_ADD,
      new IMPPackets.MusicPlayListMessage(
        name,
        imageInfo,
        pubType == PublishingType.PUBLIC,
        initAuthType == InitialAuthorityType.MEMBER,
        invitePlayers,
  ...
  private int getImportMusicsCount(
    ...
    return musicManagerBlockEntity.getImportPlayListMusicCount(
      mc.player
    );
  }
      ...
      try {
        ...
        for (AudioTrack track : pl.getRight()) {
          if (!track.getInfo().isStream) {
            ...
            var en =
              new ImportYoutubePlayListMMMonitor.YoutubePlayListEntry(
                ret.name(),
                ret.author(),
                ret.source(),
                ret.imageInfo()
