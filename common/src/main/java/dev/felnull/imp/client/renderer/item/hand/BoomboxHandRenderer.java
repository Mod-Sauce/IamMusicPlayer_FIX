...
public class BoomboxHandRenderer {
  ...
  public static void render(
    ...
    float transPr = BoomboxItem.getTransferProgress(
      stack,
      partialTicks
    );

    poseStack.pushPose();
    OERenderUtils.posePlayerArm(
      poseStack,
      arm,
      swingProgress,
      equipProgress
    ...
    poseStack.translate(
      lerpTriple(transPr, -0.85f, -1.5f, 0f) * uns,
      lerpTriple(transPr, -0.1f, -0.3f, 0.75f),
      lerpTriple(transPr, 0.32f, 0.6f, 0.5f)
    );
    OERenderUtils.poseRotateAll(
      poseStack,
      lerpTriple(transPr, -50f, -50f, -45),
      lerpTriple(transPr, 0f, 0f, -20) * uns,
      lerpTriple(transPr, -35f, -35f, 40) * uns
    );

    OERenderUtils.renderPlayerArm(
      poseStack,
      multiBufferSource,
      arm,
      packedLight
    ...
    OERenderUtils.poseTrans16(
      poseStack,
      Mth.lerp(transPr, 0.75f, -0.75) * uns,
      Mth.lerp(transPr, 0.75f, 12.25),
      Mth.lerp(transPr, 0.75f, 7.5)
    );
    OERenderUtils.poseRotateAll(
      poseStack,
      Mth.lerp(transPr, 8.42f, 0),
      Mth.lerp(transPr, 36.24f, 75) * uns,
      Mth.lerp(transPr, 2.72f, 0) * uns
    );

    OERenderUtils.renderHandItem(
      poseStack,
      multiBufferSource,
      arm,
      stack,
  ...
  private static float lerpTriple(
    float par,
    float v1,
    float v2,
    float v3
