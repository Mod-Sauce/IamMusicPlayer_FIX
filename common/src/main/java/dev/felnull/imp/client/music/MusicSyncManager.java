...
public class MusicSyncManager {
  ...
  private static final MusicSyncManager INSTANCE =
    new MusicSyncManager();
  ...
  public List<Music> getMusics(@Nullable UUID playList) {
    ...
    if (System.currentTimeMillis() - time >= 1000 * 60) {
      sendRequest(
        IMPPackets.MusicSyncType.MUSIC_BY_PLAYLIST,
        playList
      );
  ...
  public List<MusicPlayList> getMyPlayList() {
    if (
      System.currentTimeMillis() - myPlayListLastUpdateTime >=
      1000 * 60
    ) {
      sendRequest(
        IMPPackets.MusicSyncType.PLAYLIST_MY_LIST,
  ...
  public List<MusicPlayList> getCanJoinPlayList() {
    if (
      System.currentTimeMillis() - canJoinPlayListLastUpdateTime >=
      1000 * 60
    ) {
      sendRequest(
        IMPPackets.MusicSyncType.PLAYLIST_CAN_JOIN,
        mc.player.getGameProfile().getId()
      );
  ...
  private void sendRequest(IMPPackets.MusicSyncType type, UUID uuid) {
    ...
    NetworkManager.sendToServer(
      ...
      new IMPPackets.MusicSyncRequestMessage(type, uuid).toFBB()
    );
  }
