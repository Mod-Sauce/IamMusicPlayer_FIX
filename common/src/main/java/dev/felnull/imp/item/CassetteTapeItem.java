  ...
  implements DyeableLeatherItem
{
  ...
  public InteractionResultHolder<ItemStack> use(
    ...
    if (getTapePercentage(itemStack) != 0) {
      if (!level.isClientSide()) setTapePercentage(itemStack, 0f);
  ...
  public void appendHoverText(
    ...
    if (m != null) list.add(
      Component.literal(m.getName()).withStyle(ChatFormatting.GRAY)
    );
  }
  ...
  public static Music getMusic(ItemStack stack) {
    if (
      stack.getTag() != null && stack.getTag().contains("Music")
    ) return TagSerializable.loadSavedTag(
      stack.getTag().getCompound("Music"),
      new Music()
    );
    return null;
  }
  ...
  public Component getName(ItemStack stack) {
    if (getMusic(stack) != null) return Component.translatable(
      this.getDescriptionId(stack) + ".written"
    );
  ...
  public static float getTapePercentage(ItemStack stack) {
    if (stack.getTag() != null) return stack
      .getTag()
      .getFloat("TapePercentage");
    return 0;
  }

  public static ItemStack setTapePercentage(
    ItemStack stack,
    float par
  ) {
  ...
  public static boolean isSameCassetteTape(
    ItemStack stack,
    ItemStack stack2
  ) {
    ...
    if (
      stack.getItem() instanceof CassetteTapeItem &&
      stack2.getItem() instanceof CassetteTapeItem
    ) {
