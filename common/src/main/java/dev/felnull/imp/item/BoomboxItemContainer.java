...
public class BoomboxItemContainer extends ItemContainer {
  ...
  public static void openContainer(
    ...
    OEMenuUtil.openItemMenu(
      player,
      createBoomboxMenuProvider(
        stack,
        loc,
        2,
        "BoomboxItems",
  ...
  private static MenuProvider createBoomboxMenuProvider(
    ItemStack stack,
    PlayerItemLocation location,
    int size,
    String tagName,
    ...
    var con = new BoomboxItemContainer(
      stack,
      location,
      size,
      tagName,
      player -> {
        if (
          location.getItem(player).isEmpty() || stack.isEmpty()
        ) return false;
