...
public class MusicManager {
  ...
  public MusicPlayList getPlaylistByMusic(
    MinecraftServer server,
    UUID musicId
  ) {
    for (MusicPlayList value : getSaveData(server)
      .getPlayLists()
      .values()) {
      if (value.getMusicList().contains(musicId)) return value;
    }
    return null;
  }
  ...
  public void removeMusic(MinecraftServer server, UUID musicID) {
    ...
    getSaveData(server)
      ...
      .values()
      .stream()
      .map(MusicPlayList::getMusicList)
      .forEach(n -> n.remove(musicID));
    getSaveData(server).setDirty();
  }
  ...
  public List<MusicPlayList> getPlayerPlayLists(
    ServerPlayer player,
    PlayListGetType getType
  ) {
    ...
    getSaveData(player.server)
      ...
      .forEach((n, m) -> {
        if (
          getType.check(m.getAuthority().getAuthorityType(uuid))
        ) playLists.add(m);
      });
    return Collections.unmodifiableList(playLists);
  }

  public void addPlayListToPlayer(
    MinecraftServer server,
    UUID playListId,
    ServerPlayer player
  ) {
    ...
    if (pl != null && pl.getAuthority().canJoin(uuid)) {
      pl
        .getAuthority()
        .getRawAuthority()
        .put(uuid, pl.getAuthority().getInitialAuthority());
      getSaveData(server).setDirty();
    }
  }
  ...
  public void exitPlayList(
    ...
    getSaveData(server)
      ...
      .getAuthority()
      .getRawAuthority()
      .remove(player.getGameProfile().getId());
    getSaveData(server).setDirty();
  }
  ...
  public void changeAuthority(
    ...
    if (
      authorityType == AuthorityInfo.AuthorityType.INVITATION
    ) return;
    var pl = getSaveData(server).getPlayLists().get(playListId);
    if (pl == null) return;
    if (
    ...
    if (authorityType == AuthorityInfo.AuthorityType.NONE) {
      ...
      getSaveData(server)
        ...
        .getAuthority()
        .getRawAuthority()
        .put(targetPlayerId, authorityType);
    }
    getSaveData(server).setDirty();
  }

  public void addMultipleMusic(
    MinecraftServer server,
    UUID playListId,
    List<Music> musics,
    ServerPlayer player
  ...
  public void editPlayList(
    ...
    Map<UUID, AuthorityInfo.AuthorityType> naus = new HashMap<>(
      oldAuth.getRawAuthority()
    );
    List<UUID> rms = naus
      .entrySet()
      .stream()
      .filter(
        n -> n.getValue() == AuthorityInfo.AuthorityType.INVITATION
      )
      .map(Map.Entry::getKey)
      .toList();
    rms.forEach(naus::remove);
    invitePlayers.forEach(n -> {
      if (!naus.containsKey(n)) naus.put(
        n,
        AuthorityInfo.AuthorityType.INVITATION
      );
    });
    var auth = new AuthorityInfo(
      publiced,
      oldAuth.getOwner(),
      oldAuth.getOwnerName(),
      naus,
      initMember
  ...
  public static enum PlayListGetType {
    ...
    NO_JOIN(
      n -> (!n.isBan() && !n.isMoreReadOnly()) || n.isInvitation()
    );
    ...
    private PlayListGetType(
      Function<AuthorityInfo.AuthorityType, Boolean> filter
    ) {
      this.filter = filter;
    }
