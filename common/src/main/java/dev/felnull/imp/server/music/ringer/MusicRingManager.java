...
public class MusicRingManager {
  ...
  private static final MusicRingManager INSTANCE =
    new MusicRingManager();
  private final Map<ServerLevel, MusicRing> musicRings =
    new HashMap<>();
  ...
  public void onUpdate(
    ...
    getMusicRing(player.serverLevel()).onUpdate(
      player,
      uuid,
      waitUUID,
      state
  ...
  public void addReadyPlayer(
    ...
    getMusicRing(player.serverLevel()).addReadyPlayer(
      player,
      uuid,
      waitUUID,
      result,
  ...
  public boolean hasRinger(UUID uuid) {
    for (ServerLevel serverLevel : musicRings.keySet()) {
      if (hasRinger(serverLevel, uuid)) return true;
    }
    return false;
  }
  ...
  public ServerLevel getLevel(MusicRing ring) {
    for (Map.Entry<
      ...
      MusicRing
    > entry : musicRings.entrySet()) {
      if (entry.getValue() == ring) return entry.getKey();
    }
    return null;
  }
