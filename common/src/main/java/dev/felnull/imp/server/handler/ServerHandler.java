...
public class ServerHandler {
  ...
  public static void init() {
    CommandRegistrationEvent.EVENT.register(
      ServerHandler::registerCommand
    );
    LootEvent.MODIFY_LOOT_TABLE.register(
      ServerHandler::modifyLootTable
    );
  }
  ...
  public static void modifyLootTable(
    ...
    if (normal || rare) {
      var antennaPoolB = LootPool.lootPool()
        .setRolls(ConstantValue.exactly(1))
        .when(
          LootItemRandomChanceCondition.randomChance(
            rare ? 0.364364f : 0.1919810f
          )
        )
        .add(
      ...
      var djKitPoolB = LootPool.lootPool()
        .setRolls(UniformGenerator.between(1, 3))
        .when(LootItemRandomChanceCondition.randomChance(0.114514f))
        .add(
          LootItem.lootTableItem(IMPBlocks.BOOMBOX.get()).setWeight(1)
        )
        .add(
          LootItem.lootTableItem(
