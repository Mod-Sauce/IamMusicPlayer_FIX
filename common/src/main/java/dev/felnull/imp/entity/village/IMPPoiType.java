...
public class IMPPoiType {
  ...
  private static final Function<
    PoiType,
    ResourceKey<PoiType>
  > RESOURCE_KEY_CACHE = FNDataUtil.memoize(poi ->
    BuiltInRegistries.POINT_OF_INTEREST_TYPE.getResourceKey(
      poi
    ).orElseThrow()
  );

  public static final RegistrySupplier<PoiType> DJ = register(
    "dj",
    IMPBlocks.BOOMBOX,
    1,
    1
  ...
  private static RegistrySupplier<PoiType> register(
    String name,
    ...
    return POI_TYPES.register(name, () ->
      new PoiType(
        OERegisterUtils.getPoiTypeBlockStates(block.get()),
        maxTickets,
        validRange
      )
    );
