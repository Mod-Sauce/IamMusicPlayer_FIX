...
public class PlayerMusicTracker extends FixedMusicTracker {
  ...
  public PlayerMusicTracker(
    Player myPlayer,
    EntityGetter entityGetter,
    FloatSupplier deltaSupplier
  ) {
  ...
  public MusicSpeakerInfo getSpeakerInfo() {
    ...
    if (
      ...
      if (pl != null) {
        SpatialType spt = myPlayer
          .getGameProfile()
          .getId()
          .equals(playerId)
          ? SpatialType.DISABLE
          : sp.fixedInfo().spatialType();
        return new MusicSpeakerInfo(
          EntityMusicTracker.getEntityPosition(
            pl,
            deltaSupplier.getAsFloat()
          ),
          sp.volume(),
          sp.range(),
          new MusicSpeakerFixedInfo(-1, spt)
        );
      }
    }
    return sp;
