...
public class BoomboxData {
  ...
  public BoomboxData(
    CompoundTag boomboxTag,
    @NotNull BoomboxData.DataAccess access
  ) {
    this.access = access;
    if (boomboxTag != null) this.load(
      boomboxTag.getCompound("BoomBoxData"),
      true,
      true
    );
  }
  ...
  public void tick(Level level) {
    ...
    if (this.handleRaising) this.handleRaisedProgress = Mth.clamp(
      this.handleRaisedProgress + 1,
      0,
      getHandleRaisedMax()
    );
    else this.handleRaisedProgress = Mth.clamp(
      this.handleRaisedProgress - 1,
      0,
      getHandleRaisedMax()
    );

    if (this.lidOpen) this.lidOpenProgress = Mth.clamp(
      this.lidOpenProgress + 1,
      0,
      getLidOpenProgressMax()
    );
    else this.lidOpenProgress = Mth.clamp(
      this.lidOpenProgress - 1,
      0,
      getLidOpenProgressMax()
    );

    if (isUseAntenna() && isRadioStream()) this.antennaProgress =
      Mth.clamp(this.antennaProgress + 1, 0, 30);
    else this.antennaProgress = Mth.clamp(
      this.antennaProgress - 1,
      0,
      30
    );
    ...
    if (
      ...
      isUseAntenna() &&
      isAntennaExist() &&
      getAntenna().is(IMPItems.PARABOLIC_ANTENNA.get())
    ) this.parabolicAntennaProgress += 2;

    if (!level.isClientSide()) {
      if (getRinger() != null) loadingMusic =
        getRinger().isRingerWait();

      if (isPowered() && monitorType == MonitorType.OFF) monitorType =
        MonitorType.PLAYBACK;

      if (
        !isPowered() && monitorType != MonitorType.OFF
      ...
      if (
        monitorType == MonitorType.REMOTE_PLAYBACK &&
        getSelectedMusic() == null
      ) monitorType = MonitorType.REMOTE_PLAYBACK_SELECT;

      if (
        monitorType == MonitorType.RADIO && getRadioSource().isEmpty()
      ) monitorType = MonitorType.RADIO_SELECT;

      if (
        (isRadio() && !isAntennaExist()) ||
        (isRadioRemote() &&
          !IMPItemUtil.isRemotePlayBackAntenna(getAntenna())) ||
        (isRadioStream() &&
          IMPItemUtil.isRemotePlayBackAntenna(getAntenna()))
      ) if (
        (isRadioRemote() &&
          !IMPItemUtil.isRemotePlayBackAntenna(getAntenna())) ||
        (isRadioStream() && !IMPItemUtil.isRadioAntenna(getAntenna()))
      ) {
  ...
  public CompoundTag onInstruction(
    ...
    if ("buttons_press".equals(name)) {
      ...
      if (isPowered()) setMusicPositionAndRestart(
      ...
      if (isPowered()) {
        ...
        if (
          (m == MonitorType.RADIO &&
            getRadioSource() != null &&
            !getRadioSource().isEmpty()) ||
          (m == MonitorType.REMOTE_PLAYBACK &&
            getSelectedMusic() != null &&
            !getSelectedMusic().getSource().isEmpty())
        ) setRadioStartFlg(true);
      }
      return null;
    } else if ("set_radio_source".equals(name)) {
      if (isPowered()) setRadioSource(
        TagSerializable.loadSavedTag(
          data.getCompound("source"),
          new MusicSource()
        )
      );
      return null;
    } else if ("set_radio_image".equals(name)) {
      if (isPowered()) setRadioImage(
        TagSerializable.loadSavedTag(
          data.getCompound("image"),
          new ImageInfo()
        )
      );
      return null;
      ...
      if (isPowered() && isRadioRemote()) {
        if (data.contains("pl")) {
          ...
          var pl = MusicManager.getInstance()
            .getSaveData(player.server)
            .getPlayLists()
            .get(uuid);
          if (
            pl != null &&
            pl
              .getAuthority()
              .getAuthorityType(player.getGameProfile().getId())
              .isMoreReadOnly()
      ...
      if (isPowered() && isRadioRemote()) {
        if (data.contains("m")) {
          ...
          var m = MusicManager.getInstance()
            .getSaveData(player.server)
            .getMusics()
            .get(uuid);
          if (m != null) {
            var pl = MusicManager.getInstance().getPlaylistByMusic(
              player.server,
              m.getUuid()
            );
            if (
      ...
      if (isPowered() && isRadioRemote()) {
        setContinuousType(
          ContinuousType.getByName(data.getString("type"))
        );
      }
      return null;
    }
  ...
  public void onCassetteTapeChange(
    ItemStack newItem,
    ItemStack oldItem
  ) {
    ...
    if (!(newItem.isEmpty() && isLidOpen())) this.changeCassetteTape =
      true;
    update();
  }
  ...
  public CompoundTag save(
    ...
    OENbtUtils.writeUUIDMap(
      tag,
      "PlayerSelectPlaylists",
      playerSelectPlaylists
    );
    if (this.selectedMusic != null) tag.put(
      "SelectedMusic",
      this.selectedMusic.createSavedTag()
    );
    ...
    if (absolutely) {
      tag.putInt(
        "HandleRaisedProgressOld",
        this.handleRaisedProgressOld
      );
      ...
      tag.putInt(
        "ParabolicAntennaProgressOld",
        this.parabolicAntennaProgressOld
      );
      tag.putInt(
        "ParabolicAntennaProgress",
        this.parabolicAntennaProgress
      );
      tag.putInt("AntennaProgressOld", this.antennaProgressOld);
      tag.putInt("AntennaProgress", this.antennaProgress);
      tag.putBoolean(
        "NoChangeCassetteTape",
        this.noChangeCassetteTape
      );
    }
    ...
    if (absolutely || sync) {
      ...
      tag.put(
        "OldCassetteTape",
        this.oldCassetteTape.save(new CompoundTag())
      );
  ...
  public void load(
    ...
    this.monitorType = MonitorType.getByName(
      tag.getString("MonitorType")
    );
    ...
    this.radioSource = TagSerializable.loadSavedTag(
      tag.getCompound("RadioSource"),
      new MusicSource()
    );
    this.radioImage = TagSerializable.loadSavedTag(
      tag.getCompound("RadioImage"),
      new ImageInfo()
    );
    this.radioName = tag.getString("RadioName");
    this.radioAuthor = tag.getString("RadioAuthor");
    OENbtUtils.readUUIDMap(
      tag,
      "PlayerSelectPlaylists",
      playerSelectPlaylists
    );
    this.continuousType = ContinuousType.getByName(
      tag.getString("ContinuousType")
    );
    this.lastMonitorType = MonitorType.getByName(
      tag.getString("LastMonitorType")
    );

    if (tag.contains("SelectedMusic")) this.selectedMusic =
      TagSerializable.loadSavedTag(
        tag.getCompound("SelectedMusic"),
        new Music()
      );

    if (absolutely) {
      this.handleRaisedProgressOld = tag.getInt(
        "HandleRaisedProgressOld"
      );
      ...
      this.parabolicAntennaProgressOld = tag.getInt(
        "ParabolicAntennaProgressOld"
      );
      this.parabolicAntennaProgress = tag.getInt(
        "ParabolicAntennaProgress"
      );
      this.antennaProgressOld = tag.getInt("AntennaProgressOld");
      this.antennaProgress = tag.getInt("AntennaProgress");
      this.noChangeCassetteTape = tag.getBoolean(
        "NoChangeCassetteTape"
      );
    }
    ...
    if (absolutely || sync) {
      ...
      this.oldCassetteTape = ItemStack.of(
        tag.getCompound("OldCassetteTape")
      );
    ...
    if (!sync) {
      if (this.handleRaising) this.handleRaisedProgress =
        getHandleRaisedMax();
      if (this.lidOpen) this.lidOpenProgress =
        getLidOpenProgressMax();
    }
  }
  ...
  public void setMusicPosition(long position) {
    ...
    if (isMusicCassetteTapeExist() && !isRadioRemote()) {
      ...
      if (m != null) {
        var nc = CassetteTapeItem.setTapePercentage(
          getCassetteTape().copy(),
          (float) position / (float) m.getDuration()
        );
  ...
  private boolean isMusicCassetteTapeExist() {
    return (
      isCassetteTapeExist() &&
      CassetteTapeItem.getMusic(getCassetteTape()) != null
    );
  }
  ...
  public Music getCassetteTapeMusic() {
    if (
      IMPItemUtil.isCassetteTape(getCassetteTape())
  ...
  public void setSelectedPlayList(
    ...
    UUID selectedPlayList
  ) {
    if (selectedPlayList != null) {
      playerSelectPlaylists.put(
  ...
  public void setHandleRaisedProgressOld(
    int handleRaisedProgressOld
  ) {
  ...
  public static record Buttons(
    ...
    public static final Buttons EMPTY = new Buttons(
      false,
      false,
      false,
      false,
  ...
  public static enum ButtonType {
    ...
    private ButtonType(
      String name,
      Function<Buttons, Boolean> getter
    ) {
      this.name = name;
      this.component = Component.translatable(
        "imp.button.boombox." + name
      );
      this.getter = getter;
    }
