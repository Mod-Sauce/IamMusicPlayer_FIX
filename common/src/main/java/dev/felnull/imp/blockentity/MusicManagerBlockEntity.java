  ...
  extends IMPBaseEntityBlockEntity
{
  ...
  public MusicManagerBlockEntity(
    BlockPos blockPos,
    ...
    if (!level.isClientSide()) {
      blockEntity.playerData.forEach((n, m) -> {
        ...
        if (!monst.isEmpty()) {
          ...
          if (
            (blockEntity.isPowered() && type == MonitorType.OFF) ||
            (!blockEntity.isPowered() && type != MonitorType.OFF)
          ) m.putString(
            "Monitor",
            MonitorType.getDefault(blockEntity, n).getName()
          );
          ...
          if (
            m.contains("SelectedPlayList") &&
            (mm
                  .getSaveData(level.getServer())
                  .getPlayLists()
                  .get(m.getUUID("SelectedPlayList")) ==
                null ||
              !mm
                .getSaveData(level.getServer())
                .getPlayLists()
            ...
            m.contains("SelectedMusic") &&
            !mm
              .getSaveData(level.getServer())
              .getMusics()
              .containsKey(m.getUUID("SelectedMusic"))
          ) {
            m.remove("SelectedMusic");
          }
            ...
            if (m.contains("SelectedPlayList")) {
              ...
              if (pl != null) {
                rmFlg2 = !pl
                  .getAuthority()
                  .getPlayersAuthority()
                  .containsKey(m.getUUID("SelectedPlayer"));
              } else {
                rmFlg2 = true;
              }
            }
            if (rmFlg || rmFlg2) m.remove("SelectedPlayer");
          }

          if (
            type != null &&
            type.isNeedSelectPlayList() &&
            !m.contains("SelectedPlayList")
          ) {
  ...
  private void updateMonitor(
    ...
    boolean keepFlg =
      (oldM == MonitorType.ADD_MUSIC &&
        newM == MonitorType.SEARCH_MUSIC) ||
      (oldM == MonitorType.SEARCH_MUSIC &&
        newM == MonitorType.ADD_MUSIC);
    boolean keepFlg2 =
      (oldM == MonitorType.ADD_MUSIC &&
        newM == MonitorType.UPLOAD_MUSIC) ||
      (oldM == MonitorType.UPLOAD_MUSIC &&
        newM == MonitorType.ADD_MUSIC);
    boolean keepFlg3 =
      oldM != null &&
      newM != null &&
      oldM.isKeepPlayListData() &&
      newM.isKeepPlayListData();
    boolean keepFlg4 =
      oldM != null &&
      newM != null &&
      oldM.isKeepMusicData() &&
      newM.isKeepMusicData();
    ...
    if (
      oldM == MonitorType.DETAIL_PLAY_LIST &&
      newM == MonitorType.EDIT_PLAY_LIST &&
      pl != null
    ) {
      var pls = mm
        .getSaveData(level.getServer())
      ...
      if (pls != null) {
        ...
        setInitialAuthority(
          player,
          pls.getAuthority().getInitialAuthority() ==
        ...
        setPublishing(
          player,
          pls.getAuthority().isPublic() ? "public" : "private"
        );
        setInvitePlayers(
          player,
          pls
            ...
            .entrySet()
            .stream()
            .filter(n -> n.getValue().isInvitation())
    ...
    if (
      oldM == MonitorType.DETAIL_MUSIC &&
      newM == MonitorType.EDIT_MUSIC &&
      m != null
    ) {
      var pls = mm
        .getSaveData(level.getServer())
  ...
  public UUID getSelectedMusic(@NotNull ServerPlayer player) {
    var tag = getPlayerData(player);
    if (tag.contains("SelectedMusic")) return tag.getUUID(
      "SelectedMusic"
    );
    return null;
  }
  ...
  public UUID getSelectedPlayer(@NotNull ServerPlayer player) {
    var tag = getPlayerData(player);
    if (tag.contains("SelectedPlayer")) return tag.getUUID(
      "SelectedPlayer"
    );
    return null;
  }
  ...
  public UUID getSelectedPlayList(@NotNull ServerPlayer player) {
    var tag = getPlayerData(player);
    if (tag.contains("SelectedPlayList")) return tag.getUUID(
      "SelectedPlayList"
    );
    return null;
  }
  ...
  public CompoundTag getPlayerData(Player player) {
    ...
    if (!playerData.containsKey(id)) playerData.put(
      id,
      new CompoundTag()
    );
  ...
  public UUID getSelectedPlayer(Player player) {
    ...
    if (!tag.contains("SelectedPlayer")) return null;
  ...
  public UUID getSelectedMusic(Player player) {
    ...
    if (!tag.contains("SelectedMusic")) return null;
  ...
  public UUID getSelectedPlayList(Player player) {
    ...
    if (!tag.contains("SelectedPlayList")) return null;
  ...
  public MusicSource getMusicSource(Player player) {
    ...
    if (
      tag.contains("MusicSource")
    ) return TagSerializable.loadSavedTag(
      tag.getCompound("MusicSource"),
      new MusicSource()
    );
  ...
  public void setImportPlayListMusicCount(
  ...
  public void setImportPlayListAuthor(
    @NotNull ServerPlayer player,
    @NotNull String name
  ) {
  ...
  public void setImportPlayListName(
    @NotNull ServerPlayer player,
    @NotNull String name
  ) {
  ...
  public void setImportIdentifier(
    @NotNull ServerPlayer player,
  ...
  public void setSelectedMusic(
    ...
    if (type != null && type.isNeedSelectMusic()) {
      if (getPlayerData(player).contains("SelectedMusic")) {
        ...
        if (
          selectedMusic == null || !selectedMusic.equals(old)
  ...
  public void setSelectedPlayList(
    @NotNull ServerPlayer player,
    @Nullable UUID selectedPlayList
  ) {
    var type = getMonitor(player);
    if (type != null && type.isNeedSelectPlayList()) {
      if (getPlayerData(player).contains("SelectedPlayList")) {
        ...
        if (
          selectedPlayList == null || !selectedPlayList.equals(old)
        ) setMonitor(player, MonitorType.PLAY_LIST);
      }
    }
    if (selectedPlayList != null) {
      getPlayerData(player).putUUID(
        "SelectedPlayList",
        selectedPlayList
      );
    } else {
  ...
  public ImageInfo getImage(Player player) {
    ...
    if (tag.getCompound("Image").isEmpty()) return ImageInfo.EMPTY;
    return TagSerializable.loadSavedTag(
      tag.getCompound("Image"),
      new ImageInfo()
    );
  }
  ...
  public void setInitialAuthority(
    ServerPlayer player,
    String initialAuthority
  ) {
    getPlayerData(player).putString(
      "InitialAuthority",
      initialAuthority
    );
    setChanged();
  ...
  public MonitorType getMonitor(ServerPlayer player) {
    return MonitorType.getByNameOrDefault(
      getPlayerData(player).getString("Monitor"),
      this,
  ...
  public void setMonitor(ServerPlayer player, MonitorType type) {
    var oldM = MonitorType.getByNameOrDefault(
      getPlayerData(player).getString("Monitor"),
      this,
      player.getGameProfile().getId()
    );
    if (oldM != type) updateMonitor(player, type, oldM);
  ...
  public CompoundTag onInstruction(
    ...
    if ("set_monitor".equals(name)) {
      ...
      if (!mn.isEmpty()) setMonitor(
        player,
        MonitorType.getByName(mn)
      );
      return null;
    } else if ("add_playlist".equals(name)) {
      if (data.contains("playlist")) {
        var pl = data.getUUID("playlist");
        MusicManager.getInstance().addPlayListToPlayer(
          level.getServer(),
          pl,
          player
        );
      ...
      var image = TagSerializable.loadSavedTag(
      ...
      var ms = TagSerializable.loadSavedTag(
        data.getCompound("MusicSource"),
        new MusicSource()
      );
  ...
  public static enum MonitorType {
    ...
    public static MonitorType getByNameOrDefault(
      String name,
      MusicManagerBlockEntity blockEntity,
      UUID player
    ) {
    ...
    public static MonitorType getDefault(
      MusicManagerBlockEntity blockEntity,
      UUID player
    ) {
